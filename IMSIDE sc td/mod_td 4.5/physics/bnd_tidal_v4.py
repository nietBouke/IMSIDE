...
def corsol_bnd(self, zout, inp, tid_geg, indi):
    # =============================================================================
    ...
    st_cor_lft = np.sum(Bm_lft[:,:,np.newaxis] * np.cos(np.arange(self.M)[:,np.newaxis,np.newaxis]*np.pi*self.z_nd) ,0)*self.soc_sca # I think this is the correct scaling
    st_cor_rgt = np.sum(Bm_rgt[:,:,np.newaxis] * np.cos(np.arange(self.M)[:,np.newaxis,np.newaxis]*np.pi*self.z_nd) ,0)*self.soc_sca # I think this is the correct scaling
    ...
    #fluxes
    flux_lft_da = self.A_lft * np.mean(-1/4 * np.real(ut_lft*np.conj(st_cor_lft) + np.conj(ut_lft)*st_cor_lft) , axis=-1) / self.soc_sca #waarschijnlijk kan dit analytisch
    flux_rgt_da = self.A_rgt * np.mean(-1/4 * np.real(ut_rgt*np.conj(st_cor_rgt) + np.conj(ut_rgt)*st_cor_rgt) , axis=-1) / self.soc_sca #waarschijnlijk kan dit analytisch

    flux_lft_dv = self.A_lft * np.mean(-1/4 * np.real(ut_lft*np.conj(st_cor_lft) + np.conj(ut_lft)*st_cor_lft)[np.newaxis,:,:] * np.cos(self.nn*np.pi*self.z_nd) , axis=-1) / self.soc_sca #waarschijnlijk kan dit analytisch
    flux_rgt_dv = self.A_rgt * np.mean(-1/4 * np.real(ut_rgt*np.conj(st_cor_rgt) + np.conj(ut_rgt)*st_cor_rgt)[np.newaxis,:,:] * np.cos(self.nn*np.pi*self.z_nd) , axis=-1) / self.soc_sca #waarschijnlijk kan dit analytisch
...
def tidsol_bnd(self, inp, tid_geg):
    ...
    #transport
    flux_lft = self.A_lft * np.mean(1/4 * np.real(ut_lft*np.conj(st_lft) + np.conj(ut_lft)*st_lft) , axis=1) / self.soc_sca  
    #transport at vertical levels
    flux_lft_z = self.A_lft * np.mean(1/4 * np.real(ut_lft*np.conj(st_lft) + np.conj(ut_lft)*st_lft) * np.cos(self.nph[:,self.ii_all['ih_lft']]  * self.zlist[:,self.ii_all['ih_lft']]) , axis=2) / self.soc_sca  
    ...
    #transport
    flux_rgt = self.A_rgt * np.mean(1/4 * np.real(ut_rgt*np.conj(st_rgt) + np.conj(ut_rgt)*st_rgt) , axis=1) / self.soc_sca  
    #transport at vertical levels
    flux_rgt_z = self.A_rgt * np.mean(1/4 * np.real(ut_rgt*np.conj(st_rgt) + np.conj(ut_rgt)*st_rgt) * np.cos(self.nph[:,self.ii_all['ih_rgt']]  * self.zlist[:,self.ii_all['ih_rgt']]) , axis=2) / self.soc_sca  
...
def tidsol_riv(self, inp, tid_geg):
    ...
    #transport
    flux_riv = np.mean(1/4 * np.real(ut_riv*np.conj(st_riv) + np.conj(ut_riv)*st_riv)) / self.soc_sca  
...
def solu_bnd_tidal(self, ans, inp, tid_geg, indi, version):
    ...
    if version in ['D']:
        ...
        so[indi['bnd2_lft']] += (ftr_dv.T - ftl_dv.T).flatten()
        # =============================================================================
        ...
        so[indi['bnl_lft'][:,:self.M]] = sol_bc[0]
        so[indi['bnl_lft'][:,self.M:]] = sol_bc[1] 
        so[indi['bnl_rgt'][:,:self.M]] = sol_bc[2]
        so[indi['bnl_rgt'][:,self.M:]] = sol_bc[3]  
        
        #effect on fluxes
        sol_bcT = corsol_bnd(self, ans, inp, tid_geg, indi)
...
def jaco_bnd_tidal(self, ans, tid_geg, indi, version):
    ...
    if version in ['D']:
        ...
        #depth-perturbed
        jac[indi['bnd2_lft'],np.repeat(indi['i_s_3'],self.N)] +=  jtr_dv[2].T.flatten() #sb_3
        jac[indi['bnd2_lft'],np.repeat(indi['i_s_2'],self.N)] +=  jtr_dv[1].T.flatten() #sb_2
        jac[indi['bnd2_lft'],np.repeat(indi['i_s_1'],self.N)] +=  jtr_dv[0].T.flatten()  #sb_1
        
        jac[indi['bnd2_lft'],np.repeat(indi['i_s0'],self.N)] += - jtl_dv[0].T.flatten()#sb0
        jac[indi['bnd2_lft'],np.repeat(indi['i_s1'],self.N)] += - jtl_dv[1].T.flatten()#sb1
        jac[indi['bnd2_lft'],np.repeat(indi['i_s2'],self.N)] += - jtl_dv[2].T.flatten()#sb2
            
        for k in range(1,self.M):
            jac[indi['bnd2_lft'] , np.repeat(indi['i_s_1'],self.N) + k] += jtr_dv[3][k-1].T.flatten() #sn_1 in sum
            jac[indi['bnd2_lft'] , np.repeat(indi['i_s0'] ,self.N) + k] +=-jtl_dv[3][k-1].T.flatten() #sn0 in sum
        ...
        #real
        #correction
        jac[np.repeat(indi['bnl_lft'][:,:self.M],self.M) , np.tile(indi['bnl_rgt'][:,:self.M],self.M).flatten()] = np.tile(-np.cos(np.arange(self.M)[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T.flatten(),self.ndom-1)
        jac[np.repeat(indi['bnl_lft'][:,:self.M],self.M) , np.tile(indi['bnl_lft'][:,:self.M],self.M).flatten()] = np.tile(np.cos(np.arange(self.M)[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T.flatten(),self.ndom-1)
        #then other parts - lft
        jac[np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s0'],self.M**2)] += np.repeat(-jac_bc[0][0][0,:,self.z_inds].T,self.M)
        jac[np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s1'],self.M**2)] += np.repeat(-jac_bc[0][1][0,:,self.z_inds].T,self.M)
        jac[np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s2'],self.M**2)] += np.repeat(-jac_bc[0][2][0,:,self.z_inds].T,self.M)
        #jac[np.repeat(indi['bnl_lft'][:,:self.M],N) ,(np.repeat(indi['i_s0'],self.M)[:,np.newaxis]+np.arange(1,self.M)).flatten()] += -jac_bc[0][3][:,:,self.z_inds].flatten()
        
        #then other parts - rgt
        jac[np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s_1'],self.M**2)] += np.repeat(jac_bc[2][0][0,:,self.z_inds].T,self.M)
        jac[np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s_2'],self.M**2)] += np.repeat(jac_bc[2][1][0,:,self.z_inds].T,self.M)
        jac[np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s_3'],self.M**2)] += np.repeat(jac_bc[2][2][0,:,self.z_inds].T,self.M)
    
        #imaginary
        #correction
        jac[self.M+np.repeat(indi['bnl_lft'][:,:self.M],self.M) , np.tile(indi['bnl_rgt'][:,self.M:],self.M).flatten()] = np.tile(-np.cos(np.arange(self.M)[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T.flatten(),self.ndom-1)
        jac[self.M+np.repeat(indi['bnl_lft'][:,:self.M],self.M) , np.tile(indi['bnl_lft'][:,self.M:],self.M).flatten()] = np.tile(np.cos(np.arange(self.M)[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T.flatten(),self.ndom-1)
        
        #then other parts - lft
        jac[self.M+np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s0'],self.M**2)] += np.repeat(-jac_bc[1][0][0,:,self.z_inds].T,self.M)
        jac[self.M+np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s1'],self.M**2)] += np.repeat(-jac_bc[1][1][0,:,self.z_inds].T,self.M)
        jac[self.M+np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s2'],self.M**2)] += np.repeat(-jac_bc[1][2][0,:,self.z_inds].T,self.M)
    
    
        #then other parts - rgt
        jac[self.M+np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s_1'],self.M**2)] += np.repeat(jac_bc[3][0][0,:,self.z_inds].T,self.M)
        jac[self.M+np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s_2'],self.M**2)] += np.repeat(jac_bc[3][1][0,:,self.z_inds].T,self.M)
        jac[self.M+np.repeat(indi['bnl_lft'][:,:self.M],self.M) ,np.repeat(indi['i_s_3'],self.M**2)] += np.repeat(jac_bc[3][2][0,:,self.z_inds].T,self.M)
        
        # second condition: flux is constant
        #real
        #correction
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) , np.tile(indi['bnl_lft'][:,:self.M],self.M).flatten()] = (self.A_rgt[:,np.newaxis,np.newaxis] *-np.cos(np.arange(self.M)[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T).flatten() * tid_geg['eps']**0.5
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) , np.tile(indi['bnl_rgt'][:,:self.M],self.M).flatten()] = (self.A_lft[:,np.newaxis,np.newaxis] *-np.cos(np.arange(self.M)[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T).flatten() * tid_geg['eps']**0.5

        #then other parts - lft
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s0'],self.M**2)] += np.repeat(-jac_bc[0][4][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s1'],self.M**2)] += np.repeat(-jac_bc[0][5][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s2'],self.M**2)] += np.repeat(-jac_bc[0][6][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s3'],self.M**2)] += np.repeat(-jac_bc[0][7][0,:,self.z_inds].T,self.M) * tid_geg['eps']
    
        #then other parts - rgt
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s_1'],self.M**2)] += np.repeat(jac_bc[2][4][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s_2'],self.M**2)] += np.repeat(jac_bc[2][5][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s_3'],self.M**2)] += np.repeat(jac_bc[2][6][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s_4'],self.M**2)] += np.repeat(jac_bc[2][7][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        
        #imaginary
        #correction
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) , np.tile(indi['bnl_lft'][:,self.M:],self.M).flatten()] += (self.A_rgt[:,np.newaxis,np.newaxis] *-np.cos(np.arange(self.M)[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T).flatten() * tid_geg['eps']**0.5
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) , np.tile(indi['bnl_rgt'][:,self.M:],self.M).flatten()] += (self.A_lft[:,np.newaxis,np.newaxis] *-np.cos(np.arange(self.M)[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T).flatten() * tid_geg['eps']**0.5

        #then other parts - lft
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s0'],self.M**2)] += np.repeat(-jac_bc[1][4][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s1'],self.M**2)] += np.repeat(-jac_bc[1][5][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s2'],self.M**2)] += np.repeat(-jac_bc[1][6][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s3'],self.M**2)] += np.repeat(-jac_bc[1][7][0,:,self.z_inds].T,self.M) * tid_geg['eps']
    
        #then other parts - rgt
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s_1'],self.M**2)] += np.repeat(jac_bc[3][4][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s_2'],self.M**2)] += np.repeat(jac_bc[3][5][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s_3'],self.M**2)] += np.repeat(jac_bc[3][6][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        jac[self.M+np.repeat(indi['bnl_rgt'][:,:self.M],self.M) ,np.repeat(indi['i_s_4'],self.M**2)] += np.repeat(jac_bc[3][7][0,:,self.z_inds].T,self.M) * tid_geg['eps']
        ...
        for x9 in range(self.ndom-1):
            for m in range(self.M):
                ...
                #real        
                #then other parts - lft
                jac[indi['bnl_lft'][x9,m], indi['i_s0'][x9]+np.arange(1,self.M)] += -jac_bc[0][3][:,x9,self.z_inds[m]]
       
                #then other parts - rgt
                jac[indi['bnl_lft'][x9,m], indi['i_s_1'][x9] + np.arange(1,self.M)] += jac_bc[2][3][:,x9,self.z_inds[m]]
    
                #imaginary
                #then other parts - lft
                jac[indi['bnl_lft'][x9,self.M+m], indi['i_s0'][x9]+np.arange(1,self.M)] += -jac_bc[1][3][:,x9,self.z_inds[m]]
    
                #then other parts - rgt
                jac[indi['bnl_lft'][x9,self.M+m], indi['i_s_1'][x9] + np.arange(1,self.M)] += jac_bc[3][3][:,x9,self.z_inds[m]]
                # =============================================================================
                # second condition: flux is constant
                # =============================================================================
                ...
                #then other parts - lft
                jac[indi['bnl_rgt'][x9,m], indi['i_s0'][x9]+np.arange(1,self.M)] += -jac_bc[0][8][:,x9,self.z_inds[m]] * tid_geg['eps']
                jac[indi['bnl_rgt'][x9,m], indi['i_s1'][x9]+np.arange(1,self.M)] += -jac_bc[0][9][:,x9,self.z_inds[m]] * tid_geg['eps']
                jac[indi['bnl_rgt'][x9,m], indi['i_s2'][x9]+np.arange(1,self.M)] += -jac_bc[0][10][:,x9,self.z_inds[m]] * tid_geg['eps']
                
                #then other parts - rgt
                jac[indi['bnl_rgt'][x9,m], indi['i_s_1'][x9]+np.arange(1,self.M)] += jac_bc[2][8][:,x9,self.z_inds[m]] * tid_geg['eps']
                jac[indi['bnl_rgt'][x9,m], indi['i_s_2'][x9]+np.arange(1,self.M)] += jac_bc[2][9][:,x9,self.z_inds[m]] * tid_geg['eps']
                jac[indi['bnl_rgt'][x9,m], indi['i_s_3'][x9]+np.arange(1,self.M)] += jac_bc[2][10][:,x9,self.z_inds[m]] * tid_geg['eps']
                
                #imaginary
                
                #then other parts - lft
                jac[indi['bnl_rgt'][x9,self.M+m], indi['i_s0'][x9]+np.arange(1,self.M)] += -jac_bc[1][8][:,x9,self.z_inds[m]] * tid_geg['eps']
                jac[indi['bnl_rgt'][x9,self.M+m], indi['i_s1'][x9]+np.arange(1,self.M)] += -jac_bc[1][9][:,x9,self.z_inds[m]] * tid_geg['eps']
                jac[indi['bnl_rgt'][x9,self.M+m], indi['i_s2'][x9]+np.arange(1,self.M)] += -jac_bc[1][10][:,x9,self.z_inds[m]] * tid_geg['eps']
                
                #then other parts - rgt
                jac[indi['bnl_rgt'][x9,self.M+m], indi['i_s_1'][x9]+np.arange(1,self.M)] += jac_bc[3][8][:,x9,self.z_inds[m]] * tid_geg['eps']
                jac[indi['bnl_rgt'][x9,self.M+m], indi['i_s_2'][x9]+np.arange(1,self.M)] += jac_bc[3][9][:,x9,self.z_inds[m]] * tid_geg['eps']
                jac[indi['bnl_rgt'][x9,self.M+m], indi['i_s_3'][x9]+np.arange(1,self.M)] += jac_bc[3][10][:,x9,self.z_inds[m]] * tid_geg['eps']
        ...
        jac[np.repeat(indi['i_s_1'],2*self.M).reshape((self.ndom-1,2*self.M)).T , indi['bnl_lft'].T] += -Tjc[1]
        jac[np.repeat(indi['i_s_1'],2*self.M).reshape((self.ndom-1,2*self.M)).T , indi['bnl_rgt'].T] += Tjc[0]  
