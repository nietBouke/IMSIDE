...
def solu_bnd_subtidal(self, ans, pars_st, indi, version, t):
    ...
    if version in ['A','B','C','D']:
        dpf = pars_st['C12a_1'][:,np.newaxis] * (3*sn_1 - 4*sn_2 + sn_3)/(2*self.dl[self.di[ :-2],np.newaxis]) \
             + pars_st['C12b_1'] * sn_1 * (3*sn_1 - 4*sn_2 + sn_3)/(2*self.dl[self.di[ :-2],np.newaxis]) \
             + np.sum([pars_st['C12c_1'][:,n] * sn_1[:,n,np.newaxis] for n in range(self.N)] ,0) * ((3*sb_1 - 4*sb_2 + sb_3)/(2*self.dl[self.di[ :-2]]))[:,np.newaxis] \
             + pars_st['C12d_1'][t]*sn_1 \
             + np.sum([pars_st['C12e_1'][t,:,:,n] * sn_1[:,n,np.newaxis] for n in range(self.N)] ,0) \
             + pars_st['C12f_1'][t] * sb_1[:,np.newaxis] \
             + pars_st['C12g_1'] * sb_1[:,np.newaxis] * ((3*sb_1 - 4*sb_2 + sb_3)/(2*self.dl[self.di[ :-2]]))[:,np.newaxis] \
             - (pars_st['C12a0'][:,np.newaxis] * (-3*sn0 + 4*sn1 - sn2)/(2*self.dl[self.di[1:-1],np.newaxis]) \
                 + pars_st['C12b0'] * sn0 * (-3*sn0 + 4*sn1 - sn2)/(2*self.dl[self.di[1:-1],np.newaxis]) \
                 + np.sum([pars_st['C12c0'][:,n] * sn0[:,n,np.newaxis] for n in range(self.N)] ,0) * ((-3*sb0 + 4*sb1 - sb2)/(2*self.dl[self.di[1:-1]]))[:,np.newaxis] \
                 + pars_st['C12d0'][t]*sn0 \
                 + np.sum([pars_st['C12e0'][t,:,:,n] * sn0[:,n,np.newaxis] for n in range(self.N)] ,0) \
                 + pars_st['C12f0'][t] * sb0[:,np.newaxis] \
                 + pars_st['C12g0'] * sb0[:,np.newaxis] * ((-3*sb0 + 4*sb1 - sb2)/(2*self.dl[self.di[1:-1]]))[:,np.newaxis]             
                 )
...
def jaco_bnd_subtidal(self, ans, pars_st, indi, version, t):
    ...
    #depth-varying   
    if version in ['A','B','C','D']:
        jac[indi['bnd2_lft'], indi['bnd2_lft']-2*self.M] += (np.repeat(pars_st['C12a_1'],self.N).reshape((len(pars_st['C12a_1']),self.N)) * 1/(2*self.dl[self.di[ :-2],np.newaxis]) ).flatten()
        jac[indi['bnd2_lft'], indi['bnd2_lft']-self.M]   += (np.repeat(pars_st['C12a_1'],self.N).reshape((len(pars_st['C12a_1']),self.N)) *-4/(2*self.dl[self.di[ :-2],np.newaxis]) ).flatten()
        jac[indi['bnd2_lft'], indi['bnd2_lft']]     += (np.repeat(pars_st['C12a_1'],self.N).reshape((len(pars_st['C12a_1']),self.N)) * 3/(2*self.dl[self.di[ :-2],np.newaxis])).flatten() #sn_1
    
        jac[indi['bnd2_lft'], indi['bnd2_rgt']]     += (np.repeat(pars_st['C12a0'],self.N).reshape((len(pars_st['C12a0']),self.N)) * 3/(2*self.dl[self.di[1:-1],np.newaxis]) ).flatten()#sn0
        jac[indi['bnd2_lft'], indi['bnd2_rgt']+self.M]   += (np.repeat(pars_st['C12a0'],self.N).reshape((len(pars_st['C12a0']),self.N)) *-4/(2*self.dl[self.di[1:-1],np.newaxis])  ).flatten() #sn1
        jac[indi['bnd2_lft'], indi['bnd2_rgt']+2*self.M] += (np.repeat(pars_st['C12a0'],self.N).reshape((len(pars_st['C12a0']),self.N)) * 1/(2*self.dl[self.di[1:-1],np.newaxis])  ).flatten() #sn2

        jac[indi['bnd2_lft'],np.repeat(indi['i_s_3'],self.N)] += (pars_st['C12g_1'] * sb_1[:,np.newaxis] * 1/(2*self.dl[self.di[ :-2]])[:,np.newaxis] + np.sum([pars_st['C12c_1'][:,n] * sn_1[:,n,np.newaxis] for n in range(self.N)] ,0) * 1/(2*self.dl[self.di[ :-2],np.newaxis]) ).flatten() #sb_3+ jtr_dv[2].T
        jac[indi['bnd2_lft'],np.repeat(indi['i_s_2'],self.N)] += (pars_st['C12g_1'] * sb_1[:,np.newaxis] *-4/(2*self.dl[self.di[ :-2]])[:,np.newaxis] + np.sum([pars_st['C12c_1'][:,n] * sn_1[:,n,np.newaxis] for n in range(self.N)] ,0) *-4/(2*self.dl[self.di[ :-2],np.newaxis])).flatten() #sb_2 + jtr_dv[1].T
        jac[indi['bnd2_lft'],np.repeat(indi['i_s_1'],self.N)] += (pars_st['C12f_1'][t] + pars_st['C12g_1'] * sb_1[:,np.newaxis] * 3/(2*self.dl[self.di[ :-2]])[:,np.newaxis] + pars_st['C12g_1'] * ((3*sb_1 - 4*sb_2 + sb_3)/(2*self.dl[self.di[ :-2]]))[:,np.newaxis] 
                                                         + np.sum([pars_st['C12c_1'][:,n] * sn_1[:,n,np.newaxis] for n in range(self.N)] ,0) * 3/(2*self.dl[self.di[ :-2],np.newaxis])).flatten()  #sb_1 + jtr_dv[0].T
        
        jac[indi['bnd2_lft'],np.repeat(indi['i_s0'],self.N)] += (-pars_st['C12f0'][t] + pars_st['C12g0'] * sb0[:,np.newaxis] * 3/(2*self.dl[self.di[1:-1]])[:,np.newaxis] - pars_st['C12g0'] * ((-3*sb0 + 4*sb1 - sb2)/(2*self.dl[self.di[1:-1]]))[:,np.newaxis] 
                                                         + np.sum([pars_st['C12c0'][:,n] * sn0[:,n,np.newaxis] for n in range(self.N)] ,0) * 3/(2*self.dl[self.di[1:-1],np.newaxis])).flatten()#sb0 - jtl_dv[0].T
        jac[indi['bnd2_lft'],np.repeat(indi['i_s1'],self.N)] += (pars_st['C12g0'] * sb0[:,np.newaxis] *-4/(2*self.dl[self.di[1:-1]])[:,np.newaxis] + np.sum([pars_st['C12c0'][:,n] * sn0[:,n,np.newaxis] for n in range(self.N)] ,0) *-4/(2*self.dl[self.di[1:-1],np.newaxis])).flatten()#sb1 - jtl_dv[1].T
        jac[indi['bnd2_lft'],np.repeat(indi['i_s2'],self.N)] += (pars_st['C12g0'] * sb0[:,np.newaxis] * 1/(2*self.dl[self.di[1:-1]])[:,np.newaxis]+  np.sum([pars_st['C12c0'][:,n] * sn0[:,n,np.newaxis] for n in range(self.N)] ,0) * 1/(2*self.dl[self.di[1:-1],np.newaxis])).flatten()#sb2 - jtl_dv[2].T
        
        jac[indi['bnd2_lft'], indi['bnd2_lft']-2*self.M] += ( pars_st['C12b_1'] * sn_1 * 1/(2*self.dl[self.di[ :-2],np.newaxis])).flatten() #sn_3
        jac[indi['bnd2_lft'], indi['bnd2_lft']-self.M]   += ( pars_st['C12b_1'] * sn_1 *-4/(2*self.dl[self.di[ :-2],np.newaxis])).flatten() #sn_2
        jac[indi['bnd2_lft'], indi['bnd2_lft']]     += (pars_st['C12d_1'][t] + pars_st['C12b_1'] * sn_1 * 3/(2*self.dl[self.di[ :-2],np.newaxis]) + pars_st['C12b_1'] * (3*sn_1 - 4*sn_2 + sn_3)/(2*self.dl[self.di[ :-2],np.newaxis])).flatten() #sn_1
        
        jac[indi['bnd2_lft'], indi['bnd2_rgt']]     += (-pars_st['C12d0'][t] + pars_st['C12b0'] * sn0 * 3/(2*self.dl[self.di[1:-1],np.newaxis]) - pars_st['C12b0'] * (-3*sn0 + 4*sn1 - sn2)/(2*self.dl[self.di[1:-1],np.newaxis]) ).flatten()#sn0
        jac[indi['bnd2_lft'], indi['bnd2_rgt']+self.M]   += ( pars_st['C12b0'] * sn0 *-4/(2*self.dl[self.di[1:-1],np.newaxis]) ).flatten() #sn1
        jac[indi['bnd2_lft'], indi['bnd2_rgt']+2*self.M] += ( pars_st['C12b0'] * sn0 * 1/(2*self.dl[self.di[1:-1],np.newaxis]) ).flatten() #sn2
        
        for k in range(1,self.M):
            jac[indi['bnd2_lft'] , np.repeat(indi['i_s_1'],self.N) + k] += (pars_st['C12e_1'][t,:,:,k-1] + pars_st['C12c_1'][:,k-1] * ((3*sb_1 - 4*sb_2 + sb_3)/(2*self.dl[self.di[ :-2]]))[:,np.newaxis]).flatten() #sn_1 in sum + (jtr_dv[3][k-1]).T 
            jac[indi['bnd2_lft'] , np.repeat(indi['i_s0'] ,self.N) + k] += (-pars_st['C12e0'][t,:,:,k-1] -  pars_st['C12c0'][:,k-1] * ((-3*sb0 + 4*sb1 - sb2)/(2*self.dl[self.di[1:-1]]))[:,np.newaxis]).flatten() #sn0 in sum - (jtl_dv[3][k-1]).T 
