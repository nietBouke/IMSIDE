...
def indices(self):
    ...
    #inself.dices of correction
    indi['bnl_rgt'] = ( self.di3[1:-1]  *self.M+np.arange(2*self.M)[:,np.newaxis]).T
    indi['bnl_lft'] = ((self.di3[1:-1]-2)*self.M+np.arange(2*self.M)[:,np.newaxis]).T
    indi['bnl_bnd'] = np.concatenate([self.di3[0]*self.M+np.arange(2*self.M) , (self.di3[-1]-2)*self.M+np.arange(2*self.M)])   
    
    #inself.dicies of boundary
    ...
    #Inself.dices of boundary in old locaitons
    ...
    #inself.dices for salinities at boundaries     
