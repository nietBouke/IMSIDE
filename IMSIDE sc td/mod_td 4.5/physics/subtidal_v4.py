...
def subtidal_module(self):
    ...
    pars_st['C6a'] = self.Lsc*self.Kv_st*kkp**2/(2*self.H[:,np.newaxis]**2)
    ...
    pars_st['C12b_1'] = A_1[:,np.newaxis] * alf[self.di[1:-1]-1,np.newaxis]*self.soc_sca*kkp* ( g5*(kkp**2-3)/(16*kkp**3) + g4[self.di[1:-1]-1,np.newaxis]*(3-2*kkp**2)/(24*kkp**3) - g3[self.di[1:-1]-1,np.newaxis]/(4*kkp) )
    pars_st['C12c_1'] = A_1[:,np.newaxis,np.newaxis] * alf[self.di[1:-1]-1,np.newaxis,np.newaxis]*self.soc_sca*nnp* ( g5/8*(np.cos(pkn)/pkn**5*(24-12*pkn**2+pkn**4) - 24/pkn**5 + np.cos(pnk)/pnk**5*(24-12*pnk**2+pnk**4) - 24/pnk**5 )
                                      +g4[self.di[1:-1]-1,np.newaxis,np.newaxis]/6*((6-pkn**2)*np.cos(pkn)/pkn**3  + (6-pnk**2)*np.cos(pnk)/pnk**3 ) + g3[self.di[1:-1]-1,np.newaxis,np.newaxis]/2*(np.cos(pkn)/pkn + np.cos(pnk)/pnk) )
    pars_st['C12d_1'] = A_1[:,np.newaxis] * self.Lsc*u_bar[:,self.di[1:-1]-1,np.newaxis]*(0.5+0.5*g1[self.di[1:-1]-1,np.newaxis]+g2[self.di[1:-1]-1,np.newaxis]*(1/6+1/(4*kkp**2)))
    pars_st['C12e_1'] = A_1[:,np.newaxis,np.newaxis] * self.Lsc*u_bar[:,self.di[1:-1]-1,np.newaxis,np.newaxis] * (np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2)
    pars_st['C12f_1'] = A_1[:,np.newaxis] * self.Lsc*u_bar[:,self.di[1:-1]-1,np.newaxis]* 2*g2[self.di[1:-1]-1,np.newaxis]*np.cos(kkp)/kkp**2 
    pars_st['C12g_1'] = A_1[:,np.newaxis] * alf[self.di[1:-1]-1,np.newaxis]*self.soc_sca * (g4[self.di[1:-1]-1,np.newaxis]*2*np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-1)/kkp**4 - 3*np.cos(kkp)/kkp**2))
    
    pars_st['C12a0'] = A0 * 0.5*Kh[self.di[1:-1]]
    pars_st['C12b0'] = A0[:,np.newaxis] * alf[self.di[1:-1],np.newaxis]*self.soc_sca*kkp* ( g5*(kkp**2-3)/(16*kkp**3) + g4[self.di[1:-1],np.newaxis]*(3-2*kkp**2)/(24*kkp**3) - g3[self.di[1:-1],np.newaxis]/(4*kkp) )
    pars_st['C12c0'] = A0[:,np.newaxis,np.newaxis] * alf[self.di[1:-1],np.newaxis,np.newaxis]*self.soc_sca*nnp* ( g5/8*(np.cos(pkn)/pkn**5*(24-12*pkn**2+pkn**4) - 24/pkn**5 + np.cos(pnk)/pnk**5*(24-12*pnk**2+pnk**4) - 24/pnk**5 )
                                      + g4[self.di[1:-1],np.newaxis,np.newaxis]/6*((6-pkn**2)*np.cos(pkn)/pkn**3  + (6-pnk**2)*np.cos(pnk)/pnk**3 ) + g3[self.di[1:-1],np.newaxis,np.newaxis]/2*(np.cos(pkn)/pkn + np.cos(pnk)/pnk) )
    pars_st['C12d0'] = A0[:,np.newaxis] * self.Lsc*u_bar[:,self.di[1:-1],np.newaxis]*(0.5+0.5*g1[self.di[1:-1],np.newaxis]+g2[self.di[1:-1],np.newaxis]*(1/6+1/(4*kkp**2)))
    pars_st['C12e0'] = A0[:,np.newaxis,np.newaxis] * self.Lsc*u_bar[:,self.di[1:-1],np.newaxis,np.newaxis]* (np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2)
    pars_st['C12f0'] = A0[:,np.newaxis] * self.Lsc*u_bar[:,self.di[1:-1],np.newaxis]* 2*g2[self.di[1:-1],np.newaxis]*np.cos(kkp)/kkp**2 
    pars_st['C12g0'] = A0[:,np.newaxis] * alf[self.di[1:-1],np.newaxis]*self.soc_sca * (g4[self.di[1:-1],np.newaxis]*2*np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-1)/kkp**4 - 3*np.cos(kkp)/kkp**2))
