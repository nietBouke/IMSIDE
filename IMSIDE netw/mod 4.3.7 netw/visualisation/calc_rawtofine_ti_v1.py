...
def calc_output(self):
    ...
    for key in self.ch_keys:
        ...
        self.ch_outp[key]['u_st'] = Qnow[key] * self.ch_pars[key]['bH_1'][:,np.newaxis] * (self.ch_pars[key]['g1'][:,np.newaxis] + 1 + self.ch_pars[key]['g2'][:,np.newaxis] * self.z_nd**2) \
            + self.ch_pars[key]['alf'][:,np.newaxis] * self.ch_outp[key]['sb_st_x'][:,np.newaxis] * (self.ch_pars[key]['g3'][:,np.newaxis] + self.ch_pars[key]['g4'][:,np.newaxis] * self.z_nd**2 + self.ch_pars[key]['g5'] * self.z_nd**3)
            
        #Calculate transports      
        self.ch_outp[key]['TQ'] = Qnow[key]*self.ch_outp[key]['sb_st'] #transport by mean current
        self.ch_outp[key]['TE'] = np.sum(self.ch_outp[key]['sn_st']*(2*Qnow[key]*self.ch_pars[key]['g2'][:,np.newaxis]*np.cos(nnp)/nnp**2 \
                              + self.ch_pars[key]['H'][:,np.newaxis]*self.ch_pars[key]['b'][:,np.newaxis]*self.ch_pars[key]['alf'][:,np.newaxis]*self.ch_outp[key]['sb_st_x'][:,np.newaxis]*(2*self.ch_pars[key]['g4'][:,np.newaxis]\
                              * np.cos(nnp)/nnp**2 + self.ch_pars[key]['g5']*((6*np.cos(nnp)-6)/nnp**4 - 3*np.cos(nnp)/nnp**2))),1) #transport by vertically sheared current
        self.ch_outp[key]['TD'] = self.ch_pars[key]['H']*self.ch_pars[key]['b']*- self.ch_pars[key]['Kh']*self.ch_outp[key]['sb_st_x'] #transport by horizontal diffusion
        self.ch_outp[key]['TT'] = 1/4 * np.real(np.mean(np.conj(self.ch_outp[key]['s_ti'])*self.ch_outp[key]['ut'] + self.ch_outp[key]['s_ti'] * np.conj(self.ch_outp[key]['ut']),1)) * self.ch_pars[key]['H']*self.ch_pars[key]['b'] #transport by tides
        ...
        if self.ch_gegs[key]['loc x=0'][0] == 's':           
            ...
            self.ch_outp[key]['ut'] = self.ch_outp[key]['ut'][:-self.nx_sea]
        ...
        if self.ch_gegs[key]['loc x=0'][0] == 'r':
            ...
            self.ch_outp[key]['ut'] = self.ch_outp[key]['ut'][:-self.nx_riv]
