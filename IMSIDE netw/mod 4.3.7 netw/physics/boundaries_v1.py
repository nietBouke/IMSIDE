# =============================================================================
# file where functions are defined which calculate the conditions at the segment
# boundaries of the channels in a channel network
...
def func_sol_cor(self, ans, tid_inp, key):
    ...
    sol_pt3 = (- self.ch_pars[key]['eps'][:-1,np.newaxis] * (np.real(dPdx_lft) + Bm_Re_lft/np.sqrt(self.ch_pars[key]['eps'][:-1,np.newaxis])) + self.ch_pars[key]['eps'][1:,np.newaxis] * (np.real(dPdx_rgt) - Bm_Re_rgt/np.sqrt(self.ch_pars[key]['eps'][1:,np.newaxis])))[:,self.z_inds]
    sol_pt4 = (- self.ch_pars[key]['eps'][:-1,np.newaxis] * (np.imag(dPdx_lft) + Bm_Im_lft/np.sqrt(self.ch_pars[key]['eps'][:-1,np.newaxis])) + self.ch_pars[key]['eps'][1:,np.newaxis] * (np.imag(dPdx_rgt) - Bm_Im_rgt/np.sqrt(self.ch_pars[key]['eps'][1:,np.newaxis])))[:,self.z_inds]
...
def jac_bound_fix(self, key, pars_Q):
    ...
    if len(self.ch_gegs[key]['L'])>1:
        ...
        jac[inds['i_sn_1'],inds['i_sb_1_rep']] += dT_rgt_z[0]

        jac[inds['i_sn_1'],inds['i_sb0_rep']] += -dT_lft_z[0]
    ...
    #real
    #correction
    jac[inds['ibc_rgt2'] , inds['ibc_lft_rep3']] = (np.tile(-np.cos(self.m0[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T.flatten(),self.ch_pars[key]['n_seg']-1).reshape((self.M**2 , self.ch_pars[key]['n_seg']-1)) * self.ch_pars[key]['eps'][:-1]**0.5).flatten()
    jac[inds['ibc_rgt2'] , inds['ibc_rgt_rep3']] = (np.tile(-np.cos(self.m0[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T.flatten(),self.ch_pars[key]['n_seg']-1).reshape((self.M**2 , self.ch_pars[key]['n_seg']-1)) * self.ch_pars[key]['eps'][1:]**0.5).flatten()

    #then other parts - lft
    jac[inds['ibc_rgt2'] ,inds['i_sb0_rep3']] += np.repeat((-jac_bc[0][4][0,:,self.z_inds] * self.ch_pars[key]['eps'][:-1]).T,self.M)
    jac[inds['ibc_rgt2'] ,inds['i_sb1_rep3']] += np.repeat((-jac_bc[0][5][0,:,self.z_inds] * self.ch_pars[key]['eps'][:-1]).T,self.M)
    jac[inds['ibc_rgt2'] ,inds['i_sb2_rep3']] += np.repeat((-jac_bc[0][6][0,:,self.z_inds] * self.ch_pars[key]['eps'][:-1]).T,self.M)
    jac[inds['ibc_rgt2'] ,inds['i_sb3_rep3']] += np.repeat((-jac_bc[0][7][0,:,self.z_inds] * self.ch_pars[key]['eps'][:-1]).T,self.M)

    #then other parts - rgt
    jac[inds['ibc_rgt2'] ,inds['i_sb_1_rep3']] += np.repeat((jac_bc[2][4][0,:,self.z_inds] * self.ch_pars[key]['eps'][1:]).T,self.M)
    jac[inds['ibc_rgt2'] ,inds['i_sb_2_rep3']] += np.repeat((jac_bc[2][5][0,:,self.z_inds] * self.ch_pars[key]['eps'][1:]).T,self.M)
    jac[inds['ibc_rgt2'] ,inds['i_sb_3_rep3']] += np.repeat((jac_bc[2][6][0,:,self.z_inds] * self.ch_pars[key]['eps'][1:]).T,self.M)
    jac[inds['ibc_rgt2'] ,inds['i_sb_4_rep3']] += np.repeat((jac_bc[2][7][0,:,self.z_inds] * self.ch_pars[key]['eps'][1:]).T,self.M)

    #imaginary
    #correction
    jac[self.M+inds['ibc_rgt2'] , inds['ibc_lft_rep3b']] += (np.tile(-np.cos(self.m0[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T.flatten(),self.ch_pars[key]['n_seg']-1).reshape((self.M**2 , self.ch_pars[key]['n_seg']-1)) * self.ch_pars[key]['eps'][:-1]**0.5).flatten()
    jac[self.M+inds['ibc_rgt2'] , inds['ibc_rgt_rep3b']] += (np.tile(-np.cos(self.m0[:,np.newaxis]*np.pi*self.z_nd[self.z_inds]).T.flatten(),self.ch_pars[key]['n_seg']-1).reshape((self.M**2 , self.ch_pars[key]['n_seg']-1)) * self.ch_pars[key]['eps'][1:]**0.5).flatten()

    #then other parts - lft
    jac[self.M+inds['ibc_rgt2'] ,inds['i_sb0_rep3']] += np.repeat((-jac_bc[1][4][0,:,self.z_inds] * self.ch_pars[key]['eps'][:-1]).T,self.M)
    jac[self.M+inds['ibc_rgt2'] ,inds['i_sb1_rep3']] += np.repeat((-jac_bc[1][5][0,:,self.z_inds] * self.ch_pars[key]['eps'][:-1]).T,self.M)
    jac[self.M+inds['ibc_rgt2'] ,inds['i_sb2_rep3']] += np.repeat((-jac_bc[1][6][0,:,self.z_inds] * self.ch_pars[key]['eps'][:-1]).T,self.M)
    jac[self.M+inds['ibc_rgt2'] ,inds['i_sb3_rep3']] += np.repeat((-jac_bc[1][7][0,:,self.z_inds] * self.ch_pars[key]['eps'][:-1]).T,self.M)

    #then other parts - rgt
    jac[self.M+inds['ibc_rgt2'] ,inds['i_sb_1_rep3']] += np.repeat((jac_bc[3][4][0,:,self.z_inds] * self.ch_pars[key]['eps'][1:]).T,self.M)
    jac[self.M+inds['ibc_rgt2'] ,inds['i_sb_2_rep3']] += np.repeat((jac_bc[3][5][0,:,self.z_inds] * self.ch_pars[key]['eps'][1:]).T,self.M)
    jac[self.M+inds['ibc_rgt2'] ,inds['i_sb_3_rep3']] += np.repeat((jac_bc[3][6][0,:,self.z_inds] * self.ch_pars[key]['eps'][1:]).T,self.M)
    jac[self.M+inds['ibc_rgt2'] ,inds['i_sb_4_rep3']] += np.repeat((jac_bc[3][7][0,:,self.z_inds] * self.ch_pars[key]['eps'][1:]).T,self.M)
    ...
    for x9 in range(self.ch_pars[key]['n_seg']-1):
        for m in range(self.M):
            ...
            #then other parts - lft
            jac[inds['ibc_rgt'][x9,m], inds['i_sb0'][x9]+self.mm] += -jac_bc[0][8][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9]
            jac[inds['ibc_rgt'][x9,m], inds['i_sb1'][x9]+self.mm] += -jac_bc[0][9][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9]
            jac[inds['ibc_rgt'][x9,m], inds['i_sb2'][x9]+self.mm] += -jac_bc[0][10][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9]

            #then other parts - rgt
            jac[inds['ibc_rgt'][x9,m], inds['i_sb_1'][x9]+self.mm] += jac_bc[2][8][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9+1]
            jac[inds['ibc_rgt'][x9,m], inds['i_sb_2'][x9]+self.mm] += jac_bc[2][9][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9+1]
            jac[inds['ibc_rgt'][x9,m], inds['i_sb_3'][x9]+self.mm] += jac_bc[2][10][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9+1]

            #imaginary

            #then other parts - lft
            jac[inds['ibc_rgt'][x9,self.M+m], inds['i_sb0'][x9]+self.mm] += -jac_bc[1][8][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9]
            jac[inds['ibc_rgt'][x9,self.M+m], inds['i_sb1'][x9]+self.mm] += -jac_bc[1][9][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9]
            jac[inds['ibc_rgt'][x9,self.M+m], inds['i_sb2'][x9]+self.mm] += -jac_bc[1][10][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9]

            #then other parts - rgt
            jac[inds['ibc_rgt'][x9,self.M+m], inds['i_sb_1'][x9]+self.mm] += jac_bc[3][8][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9+1]
            jac[inds['ibc_rgt'][x9,self.M+m], inds['i_sb_2'][x9]+self.mm] += jac_bc[3][9][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9+1]
            jac[inds['ibc_rgt'][x9,self.M+m], inds['i_sb_3'][x9]+self.mm] += jac_bc[3][10][:,x9,self.z_inds[m]] * self.ch_pars[key]['eps'][x9+1]
...
def jac_bound_vary(self, key, ans, tid_inp, pars_Q):
    ...
    if len(self.ch_gegs[key]['L'])>1:
        ...
        #depth-varying
        jac[inds['i_sn_1'],inds['i_sb_3_rep']] += (np.sum(pars['C14c_x=0'][:,self.n0] * sn_1[:,self.n0,np.newaxis] ,1) * 1/(2*dl[self.ch_pars[key]['di'][ :-2],np.newaxis]) )  #sb_3+ jtr_dv[2].T
        jac[inds['i_sn_1'],inds['i_sb_2_rep']] += (np.sum(pars['C14c_x=0'][:,self.n0] * sn_1[:,self.n0,np.newaxis] ,1) *-4/(2*dl[self.ch_pars[key]['di'][ :-2],np.newaxis]))  #sb_2 + jtr_dv[1].T
        jac[inds['i_sn_1'],inds['i_sb_1_rep']] += (np.sum(pars['C14c_x=0'][:,self.n0] * sn_1[:,self.n0,np.newaxis] , 1) * 3/(2*dl[self.ch_pars[key]['di'][ :-2],np.newaxis]))   #sb_1 + jtr_dv[0].T

        jac[inds['i_sn_1'],inds['i_sb0_rep']] += (np.sum(pars['C14c_x=-L'][:,self.n0] * sn0[:,self.n0,np.newaxis] , 1) * 3/(2*dl[self.ch_pars[key]['di'][1:-1],np.newaxis])) #sb0 - jtl_dv[0].T
        jac[inds['i_sn_1'],inds['i_sb1_rep']] += (np.sum(pars['C14c_x=-L'][:,self.n0] * sn0[:,self.n0,np.newaxis] , 1) *-4/(2*dl[self.ch_pars[key]['di'][1:-1],np.newaxis])) #sb1 - jtl_dv[1].T
        jac[inds['i_sn_1'],inds['i_sb2_rep']] += (np.sum(pars['C14c_x=-L'][:,self.n0] * sn0[:,self.n0,np.newaxis] , 1) * 1/(2*dl[self.ch_pars[key]['di'][1:-1],np.newaxis])) #sb2 - jtl_dv[2].T
