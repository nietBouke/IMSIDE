...
def subtidal_module(self):
    ...
    for key in self.ch_keys:
        ...
        #term 2
        ch_parja[key]['C2a'] = self.ch_pars[key]['bH_1'][:,np.newaxis] * (g1/2 + g2/6 + g2/(4*kkp**2))
        ch_parja[key]['C2b'] = self.ch_pars[key]['alf']*self.soc_sca/self.Lsc * (g3/2 + g4*(1/6 + 1/(4*kkp**2)) -g5*(1/8 + 3/(8*kkp**2)) )
        ch_parja[key]['C2c'] = self.ch_pars[key]['bH_1'][:,np.newaxis,np.newaxis]*g2* ( np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2 ) 
        ch_parja[key]['C2d'] = self.soc_sca/self.Lsc*self.ch_pars[key]['alf'] * (g4*(np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2) \
        ...
        #term 3
        ch_parja[key]['C3a'] = 2*self.ch_pars[key]['bH_1'][:,np.newaxis]*g2/(kkp**2)*np.cos(kkp) 
        ch_parja[key]['C3b'] = self.ch_pars[key]['alf']*self.soc_sca/self.Lsc * ( 2*g4/kkp**2 * np.cos(kkp) - g5/kkp**4 *(6-6*np.cos(kkp) +3*kkp**2*np.cos(kkp)) )
        
        #term 4 does not exist 
        
        #term 5
        ch_parja[key]['C5a'] = self.ch_pars[key]['alf']*self.soc_sca/self.Lsc *kkp* ( -9*g5+6*g4+kkp**2*(-12*g3-4*g4+3*g5) ) / (48*kkp**3)
        ch_parja[key]['C5b'] = self.ch_pars[key]['alf']*self.soc_sca*self.ch_pars[key]['bex'][:,np.newaxis]**(-1)*kkp * ( -9*g5+6*g4+kkp**2*(-12*g3-4*g4+3*g5) ) / (48*kkp**3)    
        ch_parja[key]['C5c'] = self.ch_pars[key]['alf']*self.soc_sca/self.Lsc*nnp* ( (3*g5*np.cos(pkn)-3*g5)/pkn**5 + np.cos(pkn)/pkn**3 * (g4-1.5*g5) + np.cos(pkn)/pkn * (g5/8 - g4/6 - g3/2) 
                                + (3*g5*np.cos(pnk)-3*g5)/pnk**5 + np.cos(pnk)/pnk**3 * (g4-1.5*g5) + np.cos(pnk)/pnk * (g5/8 - g4/6 - g3/2))
        ch_parja[key]['C5d'] = self.ch_pars[key]['alf']*self.soc_sca*nnp*self.ch_pars[key]['bex'][:,np.newaxis,np.newaxis]**(-1)*( (3*g5*np.cos(pkn)-3*g5)/pkn**5 + np.cos(pkn)/pkn**3 * (g4-1.5*g5) 
                                    + np.cos(pkn)/pkn * (g5/8 - g4/6 - g3/2)  + (3*g5*np.cos(pnk)-3*g5)/pnk**5 + np.cos(pnk)/pnk**3 * (g4-1.5*g5) + np.cos(pnk)/pnk * (g5/8 - g4/6 - g3/2))
        #no wind
        ...
        ch_parja[key]['C10c'] = self.ch_pars[key]['bex'][:,np.newaxis]**(-1)*self.ch_pars[key]['alf']*self.soc_sca * ( 2*g4/nnp**2*np.cos(nnp) - g5/nnp**4 * (6-6*np.cos(nnp)+3*nnp**2*np.cos(nnp)) )
        ch_parja[key]['C10d'] = self.ch_pars[key]['alf']*self.soc_sca/self.Lsc * ( 2*g4/nnp**2*np.cos(nnp) - g5/nnp**4 * (6-6*np.cos(nnp)+3*nnp**2*np.cos(nnp)) )
        ch_parja[key]['C10e'] = (2*self.ch_pars[key]['bH_1'][:,np.newaxis]*g2) / nnp**2 * np.cos(nnp) 
        ch_parja[key]['C10f'] = self.ch_pars[key]['alf']*self.soc_sca/self.Lsc * ( 2*g4/nnp**2*np.cos(nnp) - g5/nnp**4 * (6-6*np.cos(nnp)+3*nnp**2*np.cos(nnp)) )
        ...
        ch_parja[key]['C12b_x=-L'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0]*self.ch_pars[key]['alf'] * self.soc_sca * (1/2 * g3 + (1/6 + 1/(4*kkp**2)) * g4 + (-1/8 - 3/(8*kkp**2)) * g5 )
        ch_parja[key]['C12c_x=-L'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0]*self.ch_pars[key]['alf'] * self.soc_sca * ((np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2) * g4 \
                                                                + ((3*np.cos(pkn)-3)/pkn**4 - 3/2*np.cos(pkn)/pkn**2 + (3*np.cos(pnk)-3)/pnk**4 - 3/2*np.cos(pnk)/pnk**2 ) * g5)
        ch_parja[key]['C12d_x=-L'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0]* self.Lsc * self.ch_pars[key]['bH_1'][0] * (1/2 + 1/2 * g1 + (1/6 + 1/(4*kkp**2)) * g2)
        ch_parja[key]['C12e_x=-L'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0]* self.Lsc * self.ch_pars[key]['bH_1'][0] * g2 * (np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2)
        ch_parja[key]['C12f_x=-L'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0]* self.Lsc * self.ch_pars[key]['bH_1'][0] * 2*g2*np.cos(kkp)/kkp**2 
        ch_parja[key]['C12g_x=-L'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0]* self.ch_pars[key]['alf']*self.soc_sca * (g4*2 * np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))
        
    
        ch_parja[key]['C12a_x=0'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1]*-0.5*self.ch_pars[key]['Kh'][-1]
        ch_parja[key]['C12b_x=0'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1]*self.ch_pars[key]['alf']*self.soc_sca * (1/2 * g3 + (1/6 + 1/(4*kkp**2)) * g4 + (-1/8 - 3/(8*kkp**2)) * g5 )
        ch_parja[key]['C12c_x=0'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1]*self.ch_pars[key]['alf'] * self.soc_sca * ((np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2) * g4 \
                                                                + ((3*np.cos(pkn)-3)/pkn**4 - 3/2*np.cos(pkn)/pkn**2 + (3*np.cos(pnk)-3)/pnk**4 - 3/2*np.cos(pnk)/pnk**2 ) * g5)
        ch_parja[key]['C12d_x=0'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1]* self.Lsc * self.ch_pars[key]['bH_1'][-1] * (1/2 + 1/2 * g1 + (1/6 + 1/(4*kkp**2)) * g2)
        ch_parja[key]['C12e_x=0'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1]* self.Lsc * self.ch_pars[key]['bH_1'][-1] * g2 * (np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2)
        ch_parja[key]['C12f_x=0'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1]* self.Lsc * self.ch_pars[key]['bH_1'][-1] * 2*g2*np.cos(kkp)/kkp**2 
        ch_parja[key]['C12g_x=0'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1]* self.ch_pars[key]['alf']*self.soc_sca * (g4*2 * np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))
        ...
        ch_parja[key]['C13b_x=-L'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0]*self.ch_pars[key]['bH_1'][0]*2*g2*np.cos(nnp)/nnp**2
        ch_parja[key]['C13c_x=-L'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0]*self.ch_pars[key]['alf']*self.soc_sca/self.Lsc * (2*g4*np.cos(nnp)/nnp**2 + g5*(6*np.cos(nnp)-6)/nnp**4 -g5*3*np.cos(nnp)/nnp**2 )
        ...
        ch_parja[key]['C13b_x=0'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1]*self.ch_pars[key]['bH_1'][-1]*2*g2*np.cos(nnp)/nnp**2
        ch_parja[key]['C13c_x=0'] = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1]*self.ch_pars[key]['alf']*self.soc_sca/self.Lsc * (2*g4*np.cos(nnp)/nnp**2 + g5*(6*np.cos(nnp)-6)/nnp**4 -g5*3*np.cos(nnp)/nnp**2 )
...
def sol_subtidal(self, key, ans, pars_Q):
    ...
    so[inds['xnr_mj']] += (pars['C2a'][inds['xr'],inds['j1']]*pars_Q[key]*sk_x
                         + pars['C2b'][inds['j1']] * sk_x*sb_x 
                         + np.sum(pars['C2c'][inds['xr'],inds['j1']] * (ans[inds['xnrp_mj4']]-ans[inds['xnrm_mj4']]) , 1)/(2*dl[inds['xr']]) * pars_Q[key]
                         + np.sum(pars['C2d'][inds['j1']] * (ans[inds['xnrp_mj4']]-ans[inds['xnrm_mj4']]) , 1)/(2*dl[inds['xr']]) * sb_x
                         )
    
    #term 3
    so[inds['xnr_mj']] +=  pars['C3a'][inds['xr'],inds['j1']] * pars_Q[key] * sb_x + pars['C3b'][inds['j1']] * sb_x**2
    
    #term 4 is absent
    
    #term 5
    so[inds['xnr_mj']] += (pars['C5a'][inds['j1']]*sb_xx*ans[inds['xnr_mj']] 
                        + pars['C5b'][inds['xr'],inds['j1']]*sb_x * ans[inds['xnr_mj']]  
                        + np.sum(pars['C5c'][inds['j1']]*ans[inds['xnr_mj4']] , 1) * sb_xx
    ...
    #term 6
    so[inds['xnr_mj']] += pars['C6a'][inds['j1']]*ans[inds['xnr_mj']]
    ...
    so[inds['xn_m']] +=  (pars['C10a'][inds['x']]*(ans[inds['xnp_m']]-ans[inds['xnm_m']])/(2*dl[inds['x']]) 
                        ...
                        + (ans[inds['xnp_m']] - 2*ans[inds['xn_m']] + ans[inds['xnm_m']])/(dl[inds['x']]**2) * np.sum(pars['C10d']*ans[inds['xnr_mj3']] , 1) 
                        + np.sum(pars['C10e'][inds['x']]*(ans[inds['xnrp_mj3']]-ans[inds['xnrm_mj3']]) , 1) / (2*dl[inds['x']]) * pars_Q[key] 
                        + (ans[inds['xnp_m']]-ans[inds['xnm_m']])/(2*dl[inds['x']]) * np.sum(pars['C10f']*(ans[inds['xnrp_mj3']]-ans[inds['xnrm_mj3']]) , 1)/(2*dl[inds['x']])
...
def jac_subtidal_fix(self, key, pars_Q):
    ...
    #term 6
    jac[inds['xnr_mj'],inds['xnr_mj']] += pars['C6a'][inds['j1']]
...
def jac_subtidal_vary(self, key, ans, pars_Q):
    ...
    #term 2
    jac[inds['xnr_mj'],inds['xnrm_mj']] += - pars['C2b'][inds['j1']]/(2*dl[inds['xr']]) * (ans[inds['xnrp_m']]-ans[inds['xnrm_m']])/(2*dl[inds['xr']]) 
    jac[inds['xnr_mj'],inds['xnrp_mj']] += pars['C2b'][inds['j1']]/(2*dl[inds['xr']]) * (ans[inds['xnrp_m']]-ans[inds['xnrm_m']])/(2*dl[inds['xr']]) 
    jac[inds['xnr_mj'],inds['xnrm_m']] += ( - pars['C2b'][inds['j1']]/(2*dl[inds['xr']]) * (ans[inds['xnrp_mj']]-ans[inds['xnrm_mj']])/(2*dl[inds['xr']])
                          - np.sum(pars['C2d'][inds['j1']] * (ans[inds['xnrp_mj4']]-ans[inds['xnrm_mj4']]) , 1)/(2*dl[inds['xr']])/(4*dl[inds['xr']]**2))
    jac[inds['xnr_mj'],inds['xnrp_m']] += ( pars['C2b'][inds['j1']]/(2*dl[inds['xr']]) * (ans[inds['xnrp_mj']]-ans[inds['xnrm_mj']])/(2*dl[inds['xr']]) 
                          + np.sum(pars['C2d'][inds['j1']] * (ans[inds['xnrp_mj4']]-ans[inds['xnrm_mj4']]) , 1)/(2*dl[inds['xr']])/(4*dl[inds['xr']]**2))
    jac[inds['xnr_mj2'],inds['xnrm_mk']] +=- pars['C2d'][inds['j12'],inds['k1']]*(ans[inds['xnrp_m2']]-ans[inds['xnrm_m2']])/(4*dl[inds['xr2']]**2)
    jac[inds['xnr_mj2'],inds['xnrp_mk']] +=  pars['C2d'][inds['j12'],inds['k1']]*(ans[inds['xnrp_m2']]-ans[inds['xnrm_m2']])/(4*dl[inds['xr2']]**2)
    
    
    #term 3
    jac[inds['xnr_mj'],inds['xnrm_m']] += - pars['C3b'][inds['j1']]/dl[inds['xr']] * (ans[inds['xnrp_m']]-ans[inds['xnrm_m']])/(2*dl[inds['xr']])
    jac[inds['xnr_mj'],inds['xnrp_m']] +=   pars['C3b'][inds['j1']]/dl[inds['xr']] * (ans[inds['xnrp_m']]-ans[inds['xnrm_m']])/(2*dl[inds['xr']])

    #term 4 does not exist
    #term 5
    jac[inds['xnr_mj'], inds['xnrm_m']] += ( pars['C5a'][inds['j1']]*ans[inds['xnr_mj']]/(dl[inds['xr']]**2) - pars['C5b'][inds['xr'],inds['j1']]/(2*dl[inds['xr']])*ans[inds['xnr_mj']]
                                   + np.sum(pars['C5c'][inds['j1']]*ans[inds['xnr_mj4']] , 1) /(dl[inds['xr']]**2)
                                   - np.sum(pars['C5d'][inds['xr'],inds['j1']]*ans[inds['xnr_mj4']] , 1)/(2*dl[inds['xr']]) )
    jac[inds['xnr_mj'], inds['xnr_m']]  += - 2*pars['C5a'][inds['j1']]*ans[inds['xnr_mj']]/(dl[inds['xr']]**2) -2* np.sum(pars['C5c'][inds['j1']]*ans[inds['xnr_mj4']] , 1)/(dl[inds['xr']]**2)
    jac[inds['xnr_mj'], inds['xnrp_m']] += (pars['C5a'][inds['j1']]*ans[inds['xnr_mj']]/(dl[inds['xr']]**2) + pars['C5b'][inds['xr'],inds['j1']]/(2*dl[inds['xr']])*ans[inds['xnr_mj']]
                                   + np.sum(pars['C5c'][inds['j1']]*ans[inds['xnr_mj4']] , 1)/(dl[inds['xr']]**2)
                                   + np.sum(pars['C5d'][inds['xr'],inds['j1']]*ans[inds['xnr_mj4']] , 1)/(2*dl[inds['xr']]) )
    jac[inds['xnr_mj'],inds['xnr_mj']] += ( pars['C5a'][inds['j1']]*(ans[inds['xnrp_m']]-2*ans[inds['xnr_m']]+ans[inds['xnrm_m']])/(dl[inds['xr']]**2) 
                                     + pars['C5b'][inds['xr'],inds['j1']]*(ans[inds['xnrp_m']]-ans[inds['xnrm_m']])/(2*dl[inds['xr']]) + pars['C5e'][inds['xr'],inds['j1']] )
    jac[inds['xnr_mj2'], inds['xnr_mk']] += ( pars['C5c'][inds['j12'],inds['k1']]*(ans[inds['xnrp_m2']]-2*ans[inds['xnr_m2']]+ans[inds['xnrm_m2']])/(dl[inds['xr2']]**2)
    ...
    jac[inds['xn_m'], inds['xnm_m']] += ( - 1/(2*dl[inds['x']])*np.sum(pars['C10c'][inds['x']]*ans[inds['xnr_mj3']],1) 
                                   + 1/(dl[inds['x']]**2)*np.sum(pars['C10d']*ans[inds['xnr_mj3']] , 1) 
                                   - 1/(2*dl[inds['x']]) * np.sum(pars['C10f']*(ans[inds['xnrp_mj3']]-ans[inds['xnrm_mj3']]) , 1)/(2*dl[inds['x']]) )
    jac[inds['xnr_m'], inds['xnrm_mj']]+= - pars['C10f'][inds['j1']]/(2*dl[inds['xr']])*(ans[inds['xnrp_m']]-ans[inds['xnrm_m']])/(2*dl[inds['xr']])
    jac[inds['xn_m'],inds['xn_m']]     += -2/(dl[inds['x']]**2)*np.sum(pars['C10d']*ans[inds['xnr_mj3']] , 1)  
    jac[inds['xnr_m'], inds['xnr_mj']] += ((ans[inds['xnrp_m']]-ans[inds['xnrm_m']])/(2*dl[inds['xr']]) * pars['C10c'][inds['xr'],inds['j1']] 
                                        + (ans[inds['xnrp_m']]-2*ans[inds['xnr_m']]+ans[inds['xnrm_m']])/(dl[inds['xr']]**2) * pars['C10d'][inds['j1']]  )
    jac[inds['xn_m'], inds['xnp_m']] += ( 1/(2*dl[inds['x']])*np.sum(pars['C10c'][inds['x']]*ans[inds['xnr_mj3']],1) 
                                   + 1/(dl[inds['x']]**2)*np.sum(pars['C10d']*ans[inds['xnr_mj3']] , 1) 
                                   + 1/(2*dl[inds['x']]) * np.sum(pars['C10f']*(ans[inds['xnrp_mj3']]-ans[inds['xnrm_mj3']]) , 1)/(2*dl[inds['x']]) )
    jac[inds['xnr_m'], inds['xnrp_mj']] +=  pars['C10f'][inds['j1']]/(2*dl[inds['xr']])*(ans[inds['xnrp_m']]-ans[inds['xnrm_m']])/(2*dl[inds['xr']])
