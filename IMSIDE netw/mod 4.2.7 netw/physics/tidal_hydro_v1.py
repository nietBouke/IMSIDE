...
def tide_calc(self):
    ...
    for key in self.ch_keys:  
        ...
        self.ch_pars[key]['r_t'] = (self.ch_pars[key]['Av_ti'] / (self.ch_pars[key]['sf_ti'] * self.ch_gegs[key]['H']))
        
        #parameters for equations - see Wang et al. (2021) for an explanation what they are
        self.ch_pars[key]['deA'] = (1+1j)*self.ch_gegs[key]['H']/np.sqrt(2*self.ch_pars[key]['Av_ti']/self.omega)
        self.ch_pars[key]['deK'] = (1+1j)*self.ch_gegs[key]['H']/np.sqrt(2*self.ch_pars[key]['Kv_ti']/self.omega)
    ...
    for key in self.ch_keys:
        ...
        self.ch_pars[key]['ut'] = self.g/(1j*self.omega) * self.ch_pars[key]['detadx']  * (self.ch_pars[key]['B']*np.cosh(self.ch_pars[key]['deA']*self.z_nd) - 1)
        self.ch_pars[key]['dutdx'] = self.g/(1j*self.omega) * self.ch_pars[key]['detadx2']  * (self.ch_pars[key]['B']*np.cosh(self.ch_pars[key]['deA']*self.z_nd) - 1)
        self.ch_pars[key]['wt'] = 1j*self.omega*self.ch_pars[key]['eta'] - self.g/(1j*self.omega) * (self.ch_pars[key]['detadx2'] + self.ch_pars[key]['detadx']/self.ch_pars[key]['bex'][np.newaxis,:,np.newaxis]) \
            * (self.ch_pars[key]['B']*self.ch_gegs[key]['H']/self.ch_pars[key]['deA']*np.sinh(self.ch_pars[key]['deA']*self.z_nd) - self.z_nd*self.ch_gegs[key]['H'])        
        
        self.ch_pars[key]['utb'] = (self.g/(1j*self.omega) * self.ch_pars[key]['detadx'] * (self.ch_pars[key]['B']/self.ch_pars[key]['deA']*np.sinh(self.ch_pars[key]['deA'])-1))
        self.ch_pars[key]['utp'] = (self.g/(1j*self.omega) * self.ch_pars[key]['detadx'] * self.ch_pars[key]['B'] * (np.cosh(self.ch_pars[key]['deA']*self.z_nd) - np.sinh(self.ch_pars[key]['deA'])/self.ch_pars[key]['deA']))
            
        # =============================================================================
        # coefficients for tidal salinty - new in 4.2.7
        # =============================================================================
        c1 = - self.Kv_ti/(1j*self.omega)
        st_coef = sti_coefs(self.ch_pars[key]['zlist'],c1,(self.ch_pars[key]['deA'],self.ch_gegs[key]['H'],self.ch_pars[key]['B'],self.mm[:,np.newaxis,np.newaxis] ))
        ...
        '''
        # =============================================================================
        # for boundary layer correction, jacobian parts 
        # =============================================================================
        ...
        for dom in range(self.ch_pars[key]['n_seg']):
            ...
            dstc_z = -self.m0[:,np.newaxis,np.newaxis]*np.pi/self.ch_gegs[key]['H'] * np.sin(np.pi*self.m0[:,np.newaxis,np.newaxis]*self.z_nd) * self.soc_sca
        ...
        #save boundary layer correction      
