...
def func_jac_Tbnd(self, key):  
    ...
    # =============================================================================
    #prepare
    dsdc2 = self.ch_pars[key]['c2c']
    dsdc3 = self.ch_pars[key]['c3c']
    dsdc4 = self.ch_pars[key]['c4c']
    ...
    #depth-averaged
    #derivatives for x=-L
    dT0_dsb0 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0] * np.real( ut_here[:,0] * np.conj( dsdc2 * self.go2 * detadx_here[:,0] * -3/(2*dx_here[0]) ) + np.conj(ut_here[:,0]) * dsdc2 * self.go2 * detadx_here[:,0] * -3/(2*dx_here[0]) ).mean(2)
    dT0_dsb1 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0] * np.real( ut_here[:,0] * np.conj( dsdc2 * self.go2 * detadx_here[:,0] *  4/(2*dx_here[0]) ) + np.conj(ut_here[:,0]) * dsdc2 * self.go2 * detadx_here[:,0] *  4/(2*dx_here[0]) ).mean(2)
    dT0_dsb2 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0] * np.real( ut_here[:,0] * np.conj( dsdc2 * self.go2 * detadx_here[:,0] * -1/(2*dx_here[0]) ) + np.conj(ut_here[:,0]) * dsdc2 * self.go2 * detadx_here[:,0] * -1/(2*dx_here[0]) ).mean(2)
    dT0_dsn0 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0] * np.real( ut_here[:,0] * np.conj( dsdc3 * - nph*eta_here[:,0] + dsdc4 * - nph * self.go2 * (detadx2_here[:,0] + detadx_here[:,0]/bn_here[0]) ) 
                                                           + np.conj(ut_here[:,0]) * ( dsdc3 * - nph*eta_here[:,0] + dsdc4 * - nph * self.go2 * (detadx2_here[:,0] + detadx_here[:,0]/bn_here[0]) ) ).mean(2) 
    
    #derivatives for x=0
    dT_1_dsb_1 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1] * np.real( ut_here[:,1] * np.conj( dsdc2 * self.go2 * detadx_here[:,1] *  3/(2*dx_here[1]) ) + np.conj(ut_here[:,1]) * dsdc2 * self.go2 * detadx_here[:,1] *  3/(2*dx_here[1]) ).mean(2)
    dT_1_dsb_2 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1] * np.real( ut_here[:,1] * np.conj( dsdc2 * self.go2 * detadx_here[:,1] * -4/(2*dx_here[1]) ) + np.conj(ut_here[:,1]) * dsdc2 * self.go2 * detadx_here[:,1] * -4/(2*dx_here[1]) ).mean(2)
    dT_1_dsb_3 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1] * np.real( ut_here[:,1] * np.conj( dsdc2 * self.go2 * detadx_here[:,1] *  1/(2*dx_here[1]) ) + np.conj(ut_here[:,1]) * dsdc2 * self.go2 * detadx_here[:,1] *  1/(2*dx_here[1]) ).mean(2)
    dT_1_dsn_1 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1] * np.real( ut_here[:,1] * np.conj( dsdc3 * - nph*eta_here[:,1] + dsdc4 * - nph * self.go2 * (detadx2_here[:,1] + detadx_here[:,1]/bn_here[1]) ) 
...
def func_sol_Tblc(self, zout, key):   
    ...
    #depth-averaged transport 
    flux = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][[0,-1]] * np.mean(1/4 * np.real(self.ch_pars[key]['ut'][:,[0,-1]]*np.conj(stc) + np.conj(self.ch_pars[key]['ut'][:,[0,-1]])*stc) , axis=2)[0] 
...
def func_jac_Tblc(self, key):   
    ...
    # =============================================================================
    dT_dBR = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][[0,-1]] * np.mean(1/4 * 2*np.real(self.ch_pars[key]['ut'][:,[0,-1]])*np.cos(self.npzh) , axis=2) #waarschijnlijk kan dit analytisch
    dT_dBI = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][[0,-1]] * np.mean(1/4 * 2*np.imag(self.ch_pars[key]['ut'][:,[0,-1]])*np.cos(self.npzh) , axis=2) #waarschijnlijk kan dit analytisch
...
def func_sol_Tblcz(self, zout, key):   
    ...
    flux = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][[0,-1]] * np.mean(1/4 * np.real(self.ch_pars[key]['ut'][:,[0,-1]]*np.conj(stc) + np.conj(self.ch_pars[key]['ut'][:,[0,-1]])*stc)* np.cos(self.npzh) , axis=2)
...
def func_jac_Tblcz(self, key):   
    ...
    # Jacobian for contribution to tidal transport at boundaries due to boundary correction
    # =============================================================================
    dT_dBR = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][[0,-1]] * np.mean(1/4 * 2*(np.real(self.ch_pars[key]['ut'][:,[0,-1]])*np.cos(self.npzh))[:,np.newaxis]*np.cos(self.npzh) , axis=-1) #waarschijnlijk kan dit analytisch
    dT_dBI = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][[0,-1]] * np.mean(1/4 * 2*(np.imag(self.ch_pars[key]['ut'][:,[0,-1]])*np.cos(self.npzh))[:,np.newaxis]*np.cos(self.npzh) , axis=-1)  #waarschijnlijk kan dit analytisch
...
def func_sol_Tbndz(self, tid_inp, key):   
    # =============================================================================
    # Tidal ransport at boundaries in vertical, to add to solution vector 
    # =============================================================================
    #load from inp
    st   = tid_inp['st']
    nph  = self.nn*np.pi/self.ch_gegs[key]['H']
    ...
    flux_z = self.ch_gegs[key]['H']*self.ch_pars[key]['b'][[0,-1]] * np.mean(1/4 * np.real(self.ch_pars[key]['ut'][:,[0,-1]]*np.conj(st[[0,-1]]) + np.conj(self.ch_pars[key]['ut'][:,[0,-1]])*st[[0,-1]]) * np.cos(self.npzh[1:]) , axis=2) / self.soc_sca   
...
def func_jac_Tbndz(self, key):  
    ...
    #z levels
    #derivatives for x=-L
    dTz0_dsb0 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0] * np.mean(np.real( ut_here[:,0] * np.conj( dsdc2 * self.go2 * detadx_here[:,0] * -3/(2*dx_here[0]) ) + np.conj(ut_here[:,0]) * dsdc2 * self.go2 * detadx_here[:,0] * -3/(2*dx_here[0]) ) * np.cos(self.npzh[1:]), axis=-1)
    dTz0_dsb1 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0] * np.mean(np.real( ut_here[:,0] * np.conj( dsdc2 * self.go2 * detadx_here[:,0] *  4/(2*dx_here[0]) ) + np.conj(ut_here[:,0]) * dsdc2 * self.go2 * detadx_here[:,0] *  4/(2*dx_here[0]) ) * np.cos(self.npzh[1:]), axis=-1)
    dTz0_dsb2 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0] * np.mean(np.real( ut_here[:,0] * np.conj( dsdc2 * self.go2 * detadx_here[:,0] * -1/(2*dx_here[0]) ) + np.conj(ut_here[:,0]) * dsdc2 * self.go2 * detadx_here[:,0] * -1/(2*dx_here[0]) ) * np.cos(self.npzh[1:]), axis=-1)
    dTz0_dsn0 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][0] * np.mean(np.real( ut_here[:,0] * np.conj( dsdc3 * - nph*eta_here[:,0] + dsdc4 * - nph * self.go2 * (detadx2_here[:,0] + detadx_here[:,0]/bn_here[0]) ) 
    ...
    #derivatives for x=0
    dTz_1_dsb_1 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1] * np.mean(np.real( ut_here[:,1] * np.conj( dsdc2 * self.go2 * detadx_here[:,1] *  3/(2*dx_here[1]) ) + np.conj(ut_here[:,1]) * dsdc2 * self.go2 * detadx_here[:,1] *  3/(2*dx_here[1]) ) * np.cos(self.npzh[1:]) , axis=-1)
    dTz_1_dsb_2 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1] * np.mean(np.real( ut_here[:,1] * np.conj( dsdc2 * self.go2 * detadx_here[:,1] * -4/(2*dx_here[1]) ) + np.conj(ut_here[:,1]) * dsdc2 * self.go2 * detadx_here[:,1] * -4/(2*dx_here[1]) ) * np.cos(self.npzh[1:]) , axis=-1)
    dTz_1_dsb_3 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1] * np.mean(np.real( ut_here[:,1] * np.conj( dsdc2 * self.go2 * detadx_here[:,1] *  1/(2*dx_here[1]) ) + np.conj(ut_here[:,1]) * dsdc2 * self.go2 * detadx_here[:,1] *  1/(2*dx_here[1]) ) * np.cos(self.npzh[1:]) , axis=-1)
    dTz_1_dsn_1 = 1/4* self.ch_gegs[key]['H']*self.ch_pars[key]['b'][-1] * np.mean(np.real( ut_here[:,1] * np.conj( dsdc3 * - nph*eta_here[:,1] + dsdc4 * - nph * self.go2 * (detadx2_here[:,1] + detadx_here[:,1]/bn_here[1]) ) 
...
def func_sol_blc(self, zout, ju_geg, tid_inp):
    ...
    for ch in range(3):#transport for the channels
        ...
        if ju_geg['loc'][ch] == 'loc x=-L':
            sol_pt5 += -self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]*self.eps * ( res_3ch[ch][2] + res_3ch[ch][4]/np.sqrt(self.eps) )[self.z_inds]
            sol_pt6 += -self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]*self.eps * ( res_3ch[ch][3] + res_3ch[ch][5]/np.sqrt(self.eps) )[self.z_inds]
        elif ju_geg['loc'][ch] == 'loc x=0':
            sol_pt5 += self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1]*self.eps * ( res_3ch[ch][2] -res_3ch[ch][4]/np.sqrt(self.eps) )[self.z_inds]
            sol_pt6 += self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1]*self.eps * ( res_3ch[ch][3] - res_3ch[ch][5]/np.sqrt(self.eps) )[self.z_inds]
        else: print('ERROR')
...
def func_jac_blc(self, ju_geg):
    ...
    for ch in range(3): #for all the 3 channels adjacent to the junction
        ...
        #prepare
        dsdc2 = self.ch_pars[key_here]['c2c']
        dsdc3 = self.ch_pars[key_here]['c3c']
        dsdc4 = self.ch_pars[key_here]['c4c']
        indi  = self.ch_inds[key_here]
        nph   = self.nn*np.pi/self.ch_gegs[key_here]['H']
        ...
        if ju_geg['loc'][ch] == 'loc x=-L':          
            ...
            dst0_x_dsb0 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]*self.Lsc*dsdc2 * self.go2 * (detadx2_here * -3/(2*dx_here) + detadx_here *  2/(dx_here**2) )
            dst0_x_dsb1 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]*self.Lsc*dsdc2 * self.go2 * (detadx2_here *  4/(2*dx_here) + detadx_here * -5/(dx_here**2) )
            dst0_x_dsb2 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]*self.Lsc*dsdc2 * self.go2 * (detadx2_here * -1/(2*dx_here) + detadx_here *  4/(dx_here**2) )
            dst0_x_dsb3 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]*self.Lsc*dsdc2 * self.go2 *  detadx_here * -1/(dx_here**2)
            dst0_x_dsn0 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]*self.Lsc*(dsdc3 * - nph * detadx_here + dsdc4 * -nph * self.go2 * (detadx3_here + detadx2_here/bn_here) \
                        + (dsdc3 * -nph *eta_here + dsdc4 * -nph *  self.go2 * (detadx2_here + detadx_here/bn_here) ) * -3/(2*dx_here))
            dst0_x_dsn1 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]*self.Lsc*(dsdc3 * -nph *eta_here + dsdc4 * -nph *  self.go2 * (detadx2_here + detadx_here/bn_here) ) *  4/(2*dx_here)
            dst0_x_dsn2 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]*self.Lsc*(dsdc3 * -nph *eta_here + dsdc4 * -nph *  self.go2 * (detadx2_here + detadx_here/bn_here) ) * -1/(2*dx_here)
            ...
            out.append(((np.real(dst0_dsb0[0,0]), np.real(dst0_dsb1[0,0]), np.real(dst0_dsb2[0,0]),
                   ...
                   (np.imag(dst0_dsb0[0,0]), np.imag(dst0_dsb1[0,0]), np.imag(dst0_dsb2[0,0]),
                    ...
                    , self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][0]))
            ...
            dst_1_x_dsb_1 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1] * self.Lsc*dsdc2 * self.go2 * (detadx2_here *  3/(2*dx_here) + detadx_here *  2/(dx_here**2) )
            dst_1_x_dsb_2 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1] * self.Lsc*dsdc2 * self.go2 * (detadx2_here * -4/(2*dx_here) + detadx_here * -5/(dx_here**2) )
            dst_1_x_dsb_3 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1] * self.Lsc*dsdc2 * self.go2 * (detadx2_here *  1/(2*dx_here) + detadx_here *  4/(dx_here**2) )
            dst_1_x_dsb_4 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1] * self.Lsc*dsdc2 * self.go2 *  detadx_here * -1/(dx_here**2)
            dst_1_x_dsn_1 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1] * self.Lsc*(dsdc3 * - nph * detadx_here + dsdc4 * -nph * self.go2 * (detadx3_here + detadx2_here/bn_here) \
                          + (dsdc3 * -nph *eta_here + dsdc4 * -nph *  self.go2 * (detadx2_here + detadx_here/bn_here) ) *  3/(2*dx_here))
            dst_1_x_dsn_2 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1] * self.Lsc*(dsdc3 * -nph *eta_here + dsdc4 * -nph *  self.go2 * (detadx2_here + detadx_here/bn_here) ) * -4/(2*dx_here)
            dst_1_x_dsn_3 = self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1] * self.Lsc*(dsdc3 * -nph *eta_here + dsdc4 * -nph *  self.go2 * (detadx2_here + detadx_here/bn_here) ) *  1/(2*dx_here)
            ...
            out.append(( (np.real(dst_1_dsb_1[0,0]), np.real(dst_1_dsb_2[0,0]), np.real(dst_1_dsb_3[0,0]),
               ...
               (np.imag(dst_1_dsb_1[0,0]), np.imag(dst_1_dsb_2[0,0]), np.imag(dst_1_dsb_3[0,0]),
                ...
                np.imag(dst_1_x_dsn_1[:,0]), np.imag(dst_1_x_dsn_2[:,0]), np.imag(dst_1_x_dsn_3[:,0])) 
                       , self.ch_gegs[key_here]['H']*self.ch_pars[key_here]['b'][-1]))
            ...
            for i in range(3): #calculate contributions from channels seperately 
                ...
                if locs_now[i] == 'loc x=-L':
                    temp = temp - (pars['C12a_x=-L'] * (-3*ans[ju_here['i0'][i][k]]+4*ans[ju_here['i1'][i][k]]-ans[ju_here['i2'][i][k]])/(2*self.ch_pars[key_here]['dl'][0])
                                   ...
                                   + pars['C12f_x=-L'][k-1]*pars_Q[key_here]*ans[ju_here['i0'][i][0]]
                                   + pars['C12g_x=-L'][k-1]*ans[ju_here['i0'][i][0]] * (-3*ans[ju_here['i0'][i][0]]+4*ans[ju_here['i1'][i][0]]-ans[ju_here['i2'][i][0]])/(2*self.ch_pars[key_here]['dl'][0])
                    ...
                    temp = temp + (pars['C12a_x=0'] * (ans[ju_here['i2'][i][k]]-4*ans[ju_here['i1'][i][k]]+3*ans[ju_here['i0'][i][k]])/(2*self.ch_pars[key_here]['dl'][-1])  
                                   ...
                                   + pars['C12f_x=0'][k-1]*pars_Q[key_here]*ans[ju_here['i0'][i][0]]
                                   + pars['C12g_x=0'][k-1]*ans[ju_here['i0'][i][0]]*(ans[ju_here['i2'][i][0]]-4*ans[ju_here['i1'][i][0]]+3*ans[ju_here['i0'][i][0]])/(2*self.ch_pars[key_here]['dl'][-1])  
    ...
    for j in range(1,self.n_j+1):
        ...
        for k in range(1,self.M):
            for i in range(3): #calculate contributions from channels seperately 
                ...
                if locs_now[i] == 'loc x=-L':
                    jac[ju_here['i0'][2][k],ju_here['i0'][i][0]] = - ( + pars['C12f_x=-L'][k-1]*pars_Q[key_here] + Ttid[0][k-1])
                    ...
                    jac[ju_here['i0'][2][k],ju_here['i0'][i][0]] = pars['C12f_x=0'][k-1]*pars_Q[key_here] + Ttid[4][k-1]
        ...
        # #real part
        # #channel 1
        if ju_here['loc'][0] == 'loc x=-L': sign = -1
        elif ju_here['loc'][0] == 'loc x=0': sign = 1
        
        # #afgeleides naar Bs van C1
        # jac[ju_here['obcR'][2][0]+m,ju_here['obcR'][0][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[0][2]
        # #afgeleides naar st van C1
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i0'][0][0]] = sign * self.eps * jac_bc[0][0][4][zi_here]
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i1'][0][0]] = sign * self.eps * jac_bc[0][0][5][zi_here]
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i2'][0][0]] = sign * self.eps * jac_bc[0][0][6][zi_here]
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i3'][0][0]] = sign * self.eps * jac_bc[0][0][7][zi_here]
        # jac[ju_here['obcR'][2][0]+m,ju_here['i0'][0][0]+self.mm] = sign * self.eps * jac_bc[0][0][8 ][:,zi_here]
        # jac[ju_here['obcR'][2][0]+m,ju_here['i1'][0][0]+self.mm] = sign * self.eps * jac_bc[0][0][9 ][:,zi_here]
        # jac[ju_here['obcR'][2][0]+m,ju_here['i2'][0][0]+self.mm] = sign * self.eps * jac_bc[0][0][10][:,zi_here]
           
            
        # #channel 2
        if ju_here['loc'][1] == 'loc x=-L': sign = -1
        elif ju_here['loc'][1] == 'loc x=0': sign = 1
        
        # #afgeleides naar Bs van C2
        # jac[ju_here['obcR'][2][0]+m,ju_here['obcR'][1][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[1][2]
        # #afgeleides naar st van C2
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i0'][1][0]] = sign *self.eps * jac_bc[1][0][4][zi_here]
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i1'][1][0]] = sign *self.eps * jac_bc[1][0][5][zi_here]
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i2'][1][0]] = sign *self.eps * jac_bc[1][0][6][zi_here]
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i3'][1][0]] = sign *self.eps * jac_bc[1][0][7][zi_here]
        # jac[ju_here['obcR'][2][0]+m,ju_here['i0'][1][0]+self.mm] = sign *self.eps * jac_bc[1][0][8 ][:,zi_here]
        # jac[ju_here['obcR'][2][0]+m,ju_here['i1'][1][0]+self.mm] = sign *self.eps * jac_bc[1][0][9 ][:,zi_here]
        # jac[ju_here['obcR'][2][0]+m,ju_here['i2'][1][0]+self.mm] = sign *self.eps * jac_bc[1][0][10][:,zi_here]
        
        
        # #channel 3
        if ju_here['loc'][2] == 'loc x=-L': sign = -1
        elif ju_here['loc'][2] == 'loc x=0': sign = 1
        
        # #afgeleides naar Bs van C3
        # jac[ju_here['obcR'][2][0]+m,ju_here['obcR'][2][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[2][2]
        # #afgeleides naar st van C3
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i0'][2][0]] = sign *self.eps * jac_bc[2][0][4][zi_here]
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i1'][2][0]] = sign *self.eps * jac_bc[2][0][5][zi_here]
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i2'][2][0]] = sign *self.eps * jac_bc[2][0][6][zi_here]
        jac[ju_here['obcR'][2][0]+self.m0,ju_here['i3'][2][0]] = sign *self.eps * jac_bc[2][0][7][zi_here]
        # jac[ju_here['obcR'][2][0]+m,ju_here['i0'][2][0]+self.mm] = sign *self.eps * jac_bc[2][0][8 ][:,zi_here]
        # jac[ju_here['obcR'][2][0]+m,ju_here['i1'][2][0]+self.mm] = sign *self.eps * jac_bc[2][0][9 ][:,zi_here]
        # jac[ju_here['obcR'][2][0]+m,ju_here['i2'][2][0]+self.mm] = sign *self.eps * jac_bc[2][0][10][:,zi_here]
        ...
        # #afgeleides naar Bs van C1
        # jac[ju_here['obcI'][2][0]+m,ju_here['obcI'][0][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[0][2]
        # #afgeleides naar st van C1
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i0'][0][0]] = sign * self.eps * jac_bc[0][1][4][zi_here]
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i1'][0][0]] = sign * self.eps * jac_bc[0][1][5][zi_here]
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i2'][0][0]] = sign * self.eps * jac_bc[0][1][6][zi_here]
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i3'][0][0]] = sign * self.eps * jac_bc[0][1][7][zi_here]
        # jac[ju_here['obcI'][2][0]+m,ju_here['i0'][0][0]+self.mm] = sign * self.eps * jac_bc[0][1][8 ][:,zi_here]
        # jac[ju_here['obcI'][2][0]+m,ju_here['i1'][0][0]+self.mm] = sign * self.eps * jac_bc[0][1][9 ][:,zi_here]
        # jac[ju_here['obcI'][2][0]+m,ju_here['i2'][0][0]+self.mm] = sign * self.eps * jac_bc[0][1][10][:,zi_here]
        ...
        # #afgeleides naar Bs van C2
        # jac[ju_here['obcI'][2][0]+m,ju_here['obcI'][1][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[1][2]
        # #afgeleides naar st van C2
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i0'][1][0]] = sign *self.eps * jac_bc[1][1][4][zi_here]
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i1'][1][0]] = sign *self.eps * jac_bc[1][1][5][zi_here]
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i2'][1][0]] = sign *self.eps * jac_bc[1][1][6][zi_here]
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i3'][1][0]] = sign *self.eps * jac_bc[1][1][7][zi_here]
        # jac[ju_here['obcI'][2][0]+m,ju_here['i0'][1][0]+self.mm] = sign *self.eps * jac_bc[1][1][8 ][:,zi_here]
        # jac[ju_here['obcI'][2][0]+m,ju_here['i1'][1][0]+self.mm] = sign *self.eps * jac_bc[1][1][9 ][:,zi_here]
        # jac[ju_here['obcI'][2][0]+m,ju_here['i2'][1][0]+self.mm] = sign *self.eps * jac_bc[1][1][10][:,zi_here]
        ...
        # #afgeleides naar Bs van C3
        # jac[ju_here['obcI'][2][0]+m,ju_here['obcI'][2][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[2][2]
        # #afgeleides naar st van C3
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i0'][2][0]] = sign *self.eps * jac_bc[2][1][4][zi_here]
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i1'][2][0]] = sign *self.eps * jac_bc[2][1][5][zi_here]
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i2'][2][0]] = sign *self.eps * jac_bc[2][1][6][zi_here]
        jac[ju_here['obcI'][2][0]+self.m0,ju_here['i3'][2][0]] = sign *self.eps * jac_bc[2][1][7][zi_here]
        # jac[ju_here['obcI'][2][0]+m,ju_here['i0'][2][0]+self.mm] = sign *self.eps * jac_bc[2][1][8 ][:,zi_here]
        # jac[ju_here['obcI'][2][0]+m,ju_here['i1'][2][0]+self.mm] = sign *self.eps * jac_bc[2][1][9 ][:,zi_here]
        # jac[ju_here['obcI'][2][0]+m,ju_here['i2'][2][0]+self.mm] = sign *self.eps * jac_bc[2][1][10][:,zi_here]    
        ...
        for m in range(self.M): #TODO: for now a loop, later replace with indices allocation. but that is less easy to check. 
            ...
            jac[ju_here['obcR'][2][0]+m,ju_here['obcR'][0][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[0][2]
            #afgeleides naar st van C1
            # jac[ju_here['obcR'][2][0]+m,ju_here['i0'][0][0]] = sign * self.eps * jac_bc[0][0][4][zi_here]
            # jac[ju_here['obcR'][2][0]+m,ju_here['i1'][0][0]] = sign * self.eps * jac_bc[0][0][5][zi_here]
            # jac[ju_here['obcR'][2][0]+m,ju_here['i2'][0][0]] = sign * self.eps * jac_bc[0][0][6][zi_here]
            # jac[ju_here['obcR'][2][0]+m,ju_here['i3'][0][0]] = sign * self.eps * jac_bc[0][0][7][zi_here]
            jac[ju_here['obcR'][2][0]+m,ju_here['i0'][0][0]+self.mm] = sign * self.eps * jac_bc[0][0][8 ][:,zi_here]
            jac[ju_here['obcR'][2][0]+m,ju_here['i1'][0][0]+self.mm] = sign * self.eps * jac_bc[0][0][9 ][:,zi_here]
            jac[ju_here['obcR'][2][0]+m,ju_here['i2'][0][0]+self.mm] = sign * self.eps * jac_bc[0][0][10][:,zi_here]
            ...
            jac[ju_here['obcR'][2][0]+m,ju_here['obcR'][1][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[1][2]
            #afgeleides naar st van C2
            # jac[ju_here['obcR'][2][0]+m,ju_here['i0'][1][0]] = sign *self.eps * jac_bc[1][0][4][zi_here]
            # jac[ju_here['obcR'][2][0]+m,ju_here['i1'][1][0]] = sign *self.eps * jac_bc[1][0][5][zi_here]
            # jac[ju_here['obcR'][2][0]+m,ju_here['i2'][1][0]] = sign *self.eps * jac_bc[1][0][6][zi_here]
            # jac[ju_here['obcR'][2][0]+m,ju_here['i3'][1][0]] = sign *self.eps * jac_bc[1][0][7][zi_here]
            jac[ju_here['obcR'][2][0]+m,ju_here['i0'][1][0]+self.mm] = sign *self.eps * jac_bc[1][0][8 ][:,zi_here]
            jac[ju_here['obcR'][2][0]+m,ju_here['i1'][1][0]+self.mm] = sign *self.eps * jac_bc[1][0][9 ][:,zi_here]
            jac[ju_here['obcR'][2][0]+m,ju_here['i2'][1][0]+self.mm] = sign *self.eps * jac_bc[1][0][10][:,zi_here]
            ...
            jac[ju_here['obcR'][2][0]+m,ju_here['obcR'][2][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[2][2]
            #afgeleides naar st van C3
            # jac[ju_here['obcR'][2][0]+m,ju_here['i0'][2][0]] = sign *self.eps * jac_bc[2][0][4][zi_here]
            # jac[ju_here['obcR'][2][0]+m,ju_here['i1'][2][0]] = sign *self.eps * jac_bc[2][0][5][zi_here]
            # jac[ju_here['obcR'][2][0]+m,ju_here['i2'][2][0]] = sign *self.eps * jac_bc[2][0][6][zi_here]
            # jac[ju_here['obcR'][2][0]+m,ju_here['i3'][2][0]] = sign *self.eps * jac_bc[2][0][7][zi_here]
            jac[ju_here['obcR'][2][0]+m,ju_here['i0'][2][0]+self.mm] = sign *self.eps * jac_bc[2][0][8 ][:,zi_here]
            jac[ju_here['obcR'][2][0]+m,ju_here['i1'][2][0]+self.mm] = sign *self.eps * jac_bc[2][0][9 ][:,zi_here]
            jac[ju_here['obcR'][2][0]+m,ju_here['i2'][2][0]+self.mm] = sign *self.eps * jac_bc[2][0][10][:,zi_here]
            ...
            jac[ju_here['obcI'][2][0]+m,ju_here['obcI'][0][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[0][2]
            #afgeleides naar st van C1
            # jac[ju_here['obcI'][2][0]+m,ju_here['i0'][0][0]] = sign * self.eps * jac_bc[0][1][4][zi_here]
            # jac[ju_here['obcI'][2][0]+m,ju_here['i1'][0][0]] = sign * self.eps * jac_bc[0][1][5][zi_here]
            # jac[ju_here['obcI'][2][0]+m,ju_here['i2'][0][0]] = sign * self.eps * jac_bc[0][1][6][zi_here]
            # jac[ju_here['obcI'][2][0]+m,ju_here['i3'][0][0]] = sign * self.eps * jac_bc[0][1][7][zi_here]
            jac[ju_here['obcI'][2][0]+m,ju_here['i0'][0][0]+self.mm] = sign * self.eps * jac_bc[0][1][8 ][:,zi_here]
            jac[ju_here['obcI'][2][0]+m,ju_here['i1'][0][0]+self.mm] = sign * self.eps * jac_bc[0][1][9 ][:,zi_here]
            jac[ju_here['obcI'][2][0]+m,ju_here['i2'][0][0]+self.mm] = sign * self.eps * jac_bc[0][1][10][:,zi_here]
            ...
            jac[ju_here['obcI'][2][0]+m,ju_here['obcI'][1][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[1][2]
            #afgeleides naar st van C2
            # jac[ju_here['obcI'][2][0]+m,ju_here['i0'][1][0]] = sign *self.eps * jac_bc[1][1][4][zi_here]
            # jac[ju_here['obcI'][2][0]+m,ju_here['i1'][1][0]] = sign *self.eps * jac_bc[1][1][5][zi_here]
            # jac[ju_here['obcI'][2][0]+m,ju_here['i2'][1][0]] = sign *self.eps * jac_bc[1][1][6][zi_here]
            # jac[ju_here['obcI'][2][0]+m,ju_here['i3'][1][0]] = sign *self.eps * jac_bc[1][1][7][zi_here]
            jac[ju_here['obcI'][2][0]+m,ju_here['i0'][1][0]+self.mm] = sign *self.eps * jac_bc[1][1][8 ][:,zi_here]
            jac[ju_here['obcI'][2][0]+m,ju_here['i1'][1][0]+self.mm] = sign *self.eps * jac_bc[1][1][9 ][:,zi_here]
            jac[ju_here['obcI'][2][0]+m,ju_here['i2'][1][0]+self.mm] = sign *self.eps * jac_bc[1][1][10][:,zi_here]
            ...
            jac[ju_here['obcI'][2][0]+m,ju_here['obcI'][2][0]+self.m0] = - cos_here * self.eps**0.5 * jac_bc[2][2]
            #afgeleides naar st van C3
            # jac[ju_here['obcI'][2][0]+m,ju_here['i0'][2][0]] = sign *self.eps * jac_bc[2][1][4][zi_here]
            # jac[ju_here['obcI'][2][0]+m,ju_here['i1'][2][0]] = sign *self.eps * jac_bc[2][1][5][zi_here]
            # jac[ju_here['obcI'][2][0]+m,ju_here['i2'][2][0]] = sign *self.eps * jac_bc[2][1][6][zi_here]
            # jac[ju_here['obcI'][2][0]+m,ju_here['i3'][2][0]] = sign *self.eps * jac_bc[2][1][7][zi_here]
            jac[ju_here['obcI'][2][0]+m,ju_here['i0'][2][0]+self.mm] = sign *self.eps * jac_bc[2][1][8 ][:,zi_here]
            jac[ju_here['obcI'][2][0]+m,ju_here['i1'][2][0]+self.mm] = sign *self.eps * jac_bc[2][1][9 ][:,zi_here]
            jac[ju_here['obcI'][2][0]+m,ju_here['i2'][2][0]+self.mm] = sign *self.eps * jac_bc[2][1][10][:,zi_here]       
            ...
            for i in range(3): #calculate contributions from channels seperately 
                ...
                if locs_now[i] == 'loc x=-L':
                    jac[ju_here['i0'][2][k],ju_here['i0'][i][0]] = - (-3/(2*self.ch_pars[key_here]['dl'][0]) * pars['C12b_x=-L'][k-1] * ans[ju_here['i0'][i][k]] - 3/(2*self.ch_pars[key_here]['dl'][0]) * np.sum(pars['C12c_x=-L'][k-1]*ans[ju_here['i0'][i][1:]]) \
                        - 3/(2*self.ch_pars[key_here]['dl'][0]) * pars['C12g_x=-L'][k-1]*ans[ju_here['i0'][i][0]] + pars['C12g_x=-L'][k-1] * (-3*ans[ju_here['i0'][i][0]]+4*ans[ju_here['i1'][i][0]]-ans[ju_here['i2'][i][0]])/(2*self.ch_pars[key_here]['dl'][0]) )
                    jac[ju_here['i0'][2][k],ju_here['i1'][i][0]] = - (4/(2*self.ch_pars[key_here]['dl'][0]) * pars['C12b_x=-L'][k-1] * ans[ju_here['i0'][i][k]] + 4/(2*self.ch_pars[key_here]['dl'][0])  * np.sum(pars['C12c_x=-L'][k-1]*ans[ju_here['i0'][i][1:]]) \
                        + 4/(2*self.ch_pars[key_here]['dl'][0]) * pars['C12g_x=-L'][k-1]*ans[ju_here['i0'][i][0]] )
                    jac[ju_here['i0'][2][k],ju_here['i2'][i][0]] = - (-1/(2*self.ch_pars[key_here]['dl'][0]) * pars['C12b_x=-L'][k-1] * ans[ju_here['i0'][i][k]] - 1/(2*self.ch_pars[key_here]['dl'][0])  * np.sum(pars['C12c_x=-L'][k-1]*ans[ju_here['i0'][i][1:]]) \
                        - 1/(2*self.ch_pars[key_here]['dl'][0]) * pars['C12g_x=-L'][k-1]*ans[ju_here['i0'][i][0]] )
                    ...
                    jac[ju_here['i0'][2][k],ju_here['i0'][i][0]] = 3/(2*self.ch_pars[key_here]['dl'][-1]) * pars['C12b_x=0'][k-1] * ans[ju_here['i0'][i][k]] + 3/(2*self.ch_pars[key_here]['dl'][-1]) * np.sum([pars['C12c_x=0'][k-1]*ans[ju_here['i0'][i][1:]]]) \
                        + 3/(2*self.ch_pars[key_here]['dl'][-1]) * pars['C12g_x=0'][k-1]*ans[ju_here['i0'][i][0]] + pars['C12g_x=0'][k-1]*(ans[ju_here['i2'][i][0]]-4*ans[ju_here['i1'][i][0]]+3*ans[ju_here['i0'][i][0]])/(2*self.ch_pars[key_here]['dl'][-1])
                    jac[ju_here['i0'][2][k],ju_here['i1'][i][0]] =-4/(2*self.ch_pars[key_here]['dl'][-1]) * pars['C12b_x=0'][k-1] * ans[ju_here['i0'][i][k]] - 4/(2*self.ch_pars[key_here]['dl'][-1]) * np.sum([pars['C12c_x=0'][k-1]*ans[ju_here['i0'][i][1:]]])\
                        - 4/(2*self.ch_pars[key_here]['dl'][-1]) * pars['C12g_x=0'][k-1]*ans[ju_here['i0'][i][0]]  
                    jac[ju_here['i0'][2][k],ju_here['i2'][i][0]] = 1/(2*self.ch_pars[key_here]['dl'][-1]) * pars['C12b_x=0'][k-1] * ans[ju_here['i0'][i][k]] + 1/(2*self.ch_pars[key_here]['dl'][-1]) * np.sum([pars['C12c_x=0'][k-1]*ans[ju_here['i0'][i][1:]]])\
                        + 1/(2*self.ch_pars[key_here]['dl'][-1]) * pars['C12g_x=0'][k-1]*ans[ju_here['i0'][i][0]]  
