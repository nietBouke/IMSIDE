...
def indices(self):
    ...
    for key in self.ch_keys:    
        ...
        if len(ch_inds[key]['di2']) < 3:
            ch_inds[key]['di2_x=-L'] = np.array(di2, dtype=int).reshape((2,self.ch_pars[key]['n_seg']-1))[1]
            ch_inds[key]['di2_x=0']  = np.array(di2, dtype=int).reshape((2,self.ch_pars[key]['n_seg']-1))[0]
        else: 
            ch_inds[key]['di2_x=-L'] = np.array(di2, dtype=int).reshape((2,self.ch_pars[key]['n_seg']-1))[:,1]
            ch_inds[key]['di2_x=0']  = np.array(di2, dtype=int).reshape((2,self.ch_pars[key]['n_seg']-1))[:,0]
