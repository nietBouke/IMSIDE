...
def subtidal_module(self):
    ...
    for key in self.ch_keys:
        ...
        #term 6
        ch_parja[key]['C6a'] = self.Lsc*self.ch_pars[key]['Kv']*kkp**2/(2*self.ch_pars[key]['H'][:,np.newaxis]**2)        
        ...
        if self.ch_gegs[key]['loc x=-L'][0] == "j":  
            ...
            alfb = self.g*self.Be*self.junc_gegs[self.ch_gegs[key]['loc x=-L']]['Ha']/(48*self.ch_pars[key]['Av'])
            ...
            ch_parja[key]['C12f_x=-L'] = self.ch_pars[key]['H'][0]*self.ch_pars[key]['b'][0]* self.Lsc * bHb * 2*g2[0]*np.cos(kkp)/kkp**2 
            ch_parja[key]['C12g_x=-L'] = self.ch_pars[key]['H'][0]*self.ch_pars[key]['b'][0]* alfb * self.soc_sca * (g4[0]*2 * np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))
        ...
        if self.ch_gegs[key]['loc x=0'][0] == "j": 
            #calculate local (using average depth) parameters
            alfb = self.g*self.Be*self.junc_gegs[self.ch_gegs[key]['loc x=0']]['Ha']/(48*self.ch_pars[key]['Av'])
            ...
            ch_parja[key]['C12f_x=0'] = self.ch_pars[key]['H'][-1]*self.ch_pars[key]['b'][-1]* self.Lsc * bHb * 2*g2[-1]*np.cos(kkp)/kkp**2 
            ch_parja[key]['C12g_x=0'] = self.ch_pars[key]['H'][-1]*self.ch_pars[key]['b'][-1]* alfb * self.soc_sca * (g4[-1]*2 * np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))
        ...
        alfb = self.g*self.Be*Hbnd/(48*self.ch_pars[key]['Av'])
        ...
        ch_parja[key]['C14b_x=-L'] = self.ch_pars[key]['H'][i0,np.newaxis]*self.ch_pars[key]['b'][i0,np.newaxis]*alfb[:,np.newaxis] * self.soc_sca * (1/2 * g3[i0,np.newaxis] + (1/6 + 1/(4*kkp**2)) * g4[i0,np.newaxis] + (-1/8 - 3/(8*kkp**2)) * g5 )
        ch_parja[key]['C14c_x=-L'] = self.ch_pars[key]['H'][i0,np.newaxis,np.newaxis]*self.ch_pars[key]['b'][i0,np.newaxis,np.newaxis] * alfb[:,np.newaxis,np.newaxis] * self.soc_sca * ((np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2) * g4[i0,np.newaxis,np.newaxis] \
                                                                + ((3*np.cos(pkn)-3)/pkn**4 - 3/2*np.cos(pkn)/pkn**2 + (3*np.cos(pnk)-3)/pnk**4 - 3/2*np.cos(pnk)/pnk**2 ) * g5)
        ...
        ch_parja[key]['C14f_x=-L'] = self.ch_pars[key]['H'][i0,np.newaxis]*self.ch_pars[key]['b'][i0,np.newaxis]* self.Lsc * bH0b[:,np.newaxis] * 2*g2[i0,np.newaxis]*np.cos(kkp)/kkp**2 
        ch_parja[key]['C14g_x=-L'] = self.ch_pars[key]['H'][i0,np.newaxis]*self.ch_pars[key]['b'][i0,np.newaxis]* alfb[:,np.newaxis]*self.soc_sca * (g4[i0,np.newaxis]*2 * np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))
        
        ch_parja[key]['C14a_x=0'] = self.ch_pars[key]['H'][i_1]*self.ch_pars[key]['b'][i_1]*-0.5*self.ch_pars[key]['Kh'][i_1]
        ch_parja[key]['C14b_x=0'] = self.ch_pars[key]['H'][i_1,np.newaxis]*self.ch_pars[key]['b'][i_1,np.newaxis]*alfb[:,np.newaxis]*self.soc_sca * (1/2 * g3[i_1,np.newaxis] + (1/6 + 1/(4*kkp**2)) * g4[i_1,np.newaxis] + (-1/8 - 3/(8*kkp**2)) * g5 )
        ch_parja[key]['C14c_x=0'] = self.ch_pars[key]['H'][i_1,np.newaxis,np.newaxis]*self.ch_pars[key]['b'][i_1,np.newaxis,np.newaxis]*alfb[:,np.newaxis,np.newaxis] * self.soc_sca * ((np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2) * g4[i_1,np.newaxis,np.newaxis] \
                                                                + ((3*np.cos(pkn)-3)/pkn**4 - 3/2*np.cos(pkn)/pkn**2 + (3*np.cos(pnk)-3)/pnk**4 - 3/2*np.cos(pnk)/pnk**2 ) * g5)
        ...
        ch_parja[key]['C14f_x=0'] = self.ch_pars[key]['H'][i_1,np.newaxis]*self.ch_pars[key]['b'][i_1,np.newaxis]* self.Lsc * bH_1b[:,np.newaxis] * 2*g2[i_1,np.newaxis]*np.cos(kkp)/kkp**2 
        ch_parja[key]['C14g_x=0'] = self.ch_pars[key]['H'][i_1,np.newaxis]*self.ch_pars[key]['b'][i_1,np.newaxis]* alfb[:,np.newaxis]*self.soc_sca * (g4[i_1,np.newaxis]*2 * np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))
