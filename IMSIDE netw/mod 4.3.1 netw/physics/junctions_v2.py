...
def func_sol_blc(self, zout, ju_geg, tid_inp):
    ...
    for ch in range(3):#transport for the channels
        ...
        if ju_geg['loc'][ch] == 'loc x=-L':
            sol_pt5 += -self.ch_pars[key_here]['H'][0]*self.ch_pars[key_here]['b'][0]*self.eps * ( res_3ch[ch][2] + res_3ch[ch][4]/np.sqrt(self.eps) )[self.z_inds]
            sol_pt6 += -self.ch_pars[key_here]['H'][0]*self.ch_pars[key_here]['b'][0]*self.eps * ( res_3ch[ch][3] + res_3ch[ch][5]/np.sqrt(self.eps) )[self.z_inds]
        elif ju_geg['loc'][ch] == 'loc x=0':
            sol_pt5 += self.ch_pars[key_here]['H'][-1]*self.ch_pars[key_here]['b'][-1]*self.eps * ( res_3ch[ch][2] - res_3ch[ch][4]/np.sqrt(self.eps) )[self.z_inds]
            sol_pt6 += self.ch_pars[key_here]['H'][-1]*self.ch_pars[key_here]['b'][-1]*self.eps * ( res_3ch[ch][3] - res_3ch[ch][5]/np.sqrt(self.eps) )[self.z_inds]
        else: print('ERROR')
