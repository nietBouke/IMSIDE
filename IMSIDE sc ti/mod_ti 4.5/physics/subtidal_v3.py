...
def subtidal_module(self):
    ...
    pars_st['C12b_1'] = A_1[:,np.newaxis] * alf[self.di[1:-1]-1,np.newaxis] * self.soc * (1/2 * g3[self.di[1:-1]-1,np.newaxis] + (1/6 + 1/(4*kkp**2)) * g4[self.di[1:-1]-1,np.newaxis] + (-1/8 - 3/(8*kkp**2)) * g5 )
    pars_st['C12c_1'] = A_1[:,np.newaxis,np.newaxis] * alf[self.di[1:-1]-1,np.newaxis,np.newaxis] * self.soc * ((np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2) * g4[self.di[1:-1]-1,np.newaxis,np.newaxis] \
    ...
    pars_st['C12g_1'] = A_1[:,np.newaxis] * alf[self.di[1:-1]-1,np.newaxis]*self.soc * (g4[self.di[1:-1]-1,np.newaxis]*2 * np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))

    pars_st['C12a0'] = A0 * 0.5*-Kh[self.di[1:-1]]
    pars_st['C12b0'] = A0[:,np.newaxis] * alf[self.di[1:-1],np.newaxis]*self.soc * (1/2 * g3[self.di[1:-1],np.newaxis] + (1/6 + 1/(4*kkp**2)) * g4[self.di[1:-1],np.newaxis] + (-1/8 - 3/(8*kkp**2)) * g5 )
    pars_st['C12c0'] = A0[:,np.newaxis,np.newaxis] * alf[self.di[1:-1],np.newaxis,np.newaxis] * self.soc * ((np.cos(pkn)/pkn**2 + np.cos(pnk)/pnk**2) * g4[self.di[1:-1],np.newaxis,np.newaxis] \
                                                            + ((3*np.cos(pkn)-3)/pkn**4 - 3/2*np.cos(pkn)/pkn**2 + (3*np.cos(pnk)-3)/pnk**4 - 3/2*np.cos(pnk)/pnk**2 ) * g5)
    ...
    pars_st['C12g0'] = A0[:,np.newaxis] * alf[self.di[1:-1],np.newaxis]*self.soc * (g4[self.di[1:-1],np.newaxis]*2 * np.cos(kkp)/kkp**2 + g5*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))
