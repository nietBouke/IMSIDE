...
def solu_bnd_subtidal(self, ans, pars_st, indi, version):
    ...
    if version in ['A','B','C','D']:
        dpf = pars_st['C12a_1'][:,np.newaxis] * (3*sn_1 - 4*sn_2 + sn_3)/(2*self.dl[self.di[ :-2],np.newaxis]) \
             ...
             + pars_st['C12d_1']*sn_1 \
             + np.sum([pars_st['C12e_1'][:,:,n] * sn_1[:,n,np.newaxis] for n in range(self.N)] ,0) \
             + pars_st['C12f_1'] * sb_1[:,np.newaxis] \
             ...
             - (pars_st['C12a0'][:,np.newaxis] * (-3*sn0 + 4*sn1 - sn2)/(2*self.dl[self.di[1:-1],np.newaxis]) \
                 ...
                 + pars_st['C12d0']*sn0 \
                 + np.sum([pars_st['C12e0'][:,:,n] * sn0[:,n,np.newaxis] for n in range(self.N)] ,0) \
                 + pars_st['C12f0'] * sb0[:,np.newaxis] \
...
def jaco_bnd_subtidal(self, ans, pars_st, indi, version):
    ...
    if version in ['A','B','C','D']:
        ...
        jac[indi['bnd2_lft'],np.repeat(indi['i_s_1'],self.N)] += (pars_st['C12f_1'] + pars_st['C12g_1'] * sb_1[:,np.newaxis] * 3/(2*self.dl[self.di[ :-2]])[:,np.newaxis] + pars_st['C12g_1'] * ((3*sb_1 - 4*sb_2 + sb_3)/(2*self.dl[self.di[ :-2]]))[:,np.newaxis] 
        ...
        jac[indi['bnd2_lft'],np.repeat(indi['i_s0'],self.N)] += (-pars_st['C12f0'] + pars_st['C12g0'] * sb0[:,np.newaxis] * 3/(2*self.dl[self.di[1:-1]])[:,np.newaxis] - pars_st['C12g0'] * ((-3*sb0 + 4*sb1 - sb2)/(2*self.dl[self.di[1:-1]]))[:,np.newaxis] 
        ...
        jac[indi['bnd2_lft'], indi['bnd2_lft']]     += (pars_st['C12d_1'] + pars_st['C12b_1'] * sn_1 * 3/(2*self.dl[self.di[ :-2],np.newaxis]) + pars_st['C12b_1'] * (3*sn_1 - 4*sn_2 + sn_3)/(2*self.dl[self.di[ :-2],np.newaxis])).flatten() #sn_1
        
        jac[indi['bnd2_lft'], indi['bnd2_rgt']]     += (-pars_st['C12d0'] + pars_st['C12b0'] * sn0 * 3/(2*self.dl[self.di[1:-1],np.newaxis]) - pars_st['C12b0'] * (-3*sn0 + 4*sn1 - sn2)/(2*self.dl[self.di[1:-1],np.newaxis]) ).flatten()#sn0
        ...
        for k in range(1,self.M):
            jac[indi['bnd2_lft'] , np.repeat(indi['i_s_1'],self.N) + k] += (pars_st['C12e_1'][:,:,k-1] + pars_st['C12c_1'][:,k-1] * ((3*sb_1 - 4*sb_2 + sb_3)/(2*self.dl[self.di[ :-2]]))[:,np.newaxis]).flatten() #sn_1 in sum + (jtr_dv[3][k-1]).T 
            jac[indi['bnd2_lft'] , np.repeat(indi['i_s0'] ,self.N) + k] += (-pars_st['C12e0'][:,:,k-1] -  pars_st['C12c0'][:,k-1] * ((-3*sb0 + 4*sb1 - sb2)/(2*self.dl[self.di[1:-1]]))[:,np.newaxis]).flatten() #sn0 in sum - (jtl_dv[3][k-1]).T 
