...
def solu_bnd_subtidal(self, ans, pars_st, indi, version):
    ...
    if version in ['A','B','C','D']:
        dpf = pars_st['C15a_1'][:,np.newaxis] * (3*sn_1 - 4*sn_2 + sn_3)/(2*self.dl[self.di[ :-2],np.newaxis]) \
             ...
             + pars_st['C15f_1'] * sb_1[:,np.newaxis] \
...
def jaco_bnd_subtidal(self, ans, pars_st, indi, version):
    ...
    if version in ['A','B','C','D']:
        ...
        jac[indi['bnd2_lft'],np.repeat(indi['i_s_1'],self.N)] += (pars_st['C15f_1'] + pars_st['C15g_1'] * sb_1[:,np.newaxis] * 3/(2*self.dl[self.di[ :-2]])[:,np.newaxis] + pars_st['C15g_1'] * ((3*sb_1 - 4*sb_2 + sb_3)/(2*self.dl[self.di[ :-2]]))[:,np.newaxis] 
        ...
        jac[indi['bnd2_lft'],np.repeat(indi['i_s0'],self.N)] += (-pars_st['C15f0'] + pars_st['C15g0'] * sb0[:,np.newaxis] * 3/(2*self.dl[self.di[1:-1]])[:,np.newaxis] - pars_st['C15g0'] * ((-3*sb0 + 4*sb1 - sb2)/(2*self.dl[self.di[1:-1]]))[:,np.newaxis] 
        ...
        jac[indi['bnd2_lft'], indi['bnd2_lft']]     += (pars_st['C15d_1'] + pars_st['C15b_1'] * sn_1 * 3/(2*self.dl[self.di[ :-2],np.newaxis]) + pars_st['C15b_1'] * (3*sn_1 - 4*sn_2 + sn_3)/(2*self.dl[self.di[ :-2],np.newaxis])).flatten() #sn_1
        
        jac[indi['bnd2_lft'], indi['bnd2_rgt']]     += (-pars_st['C15d0'] + pars_st['C15b0'] * sn0 * 3/(2*self.dl[self.di[1:-1],np.newaxis]) - pars_st['C15b0'] * (-3*sn0 + 4*sn1 - sn2)/(2*self.dl[self.di[1:-1],np.newaxis]) ).flatten()#sn0
        ...
        for k in range(1,self.M):
            jac[indi['bnd2_lft'] , np.repeat(indi['i_s_1'],self.N) + k] += (pars_st['C15e_1'][:,:,k-1] + pars_st['C15c_1'][:,k-1] * ((3*sb_1 - 4*sb_2 + sb_3)/(2*self.dl[self.di[ :-2]]))[:,np.newaxis]).flatten() #sn_1 in sum + (jtr_dv[3][k-1]).T 
            jac[indi['bnd2_lft'] , np.repeat(indi['i_s0'] ,self.N) + k] += (-pars_st['C15e0'][:,:,k-1] -  pars_st['C15c0'][:,k-1] * ((-3*sb0 + 4*sb1 - sb2)/(2*self.dl[self.di[1:-1]]))[:,np.newaxis]).flatten() #sn0 in sum - (jtl_dv[3][k-1]).T 
