...
def subtidal_module(self):
    ...
    pars_st['C15g_1'] = A_1[:,np.newaxis] * alfb[:,np.newaxis]*self.soc * (g4b[:,np.newaxis]*2 * np.cos(kkp)/kkp**2 + g5b*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))
    ...
    pars_st['C15g0'] = A0[:,np.newaxis] * alfb[:,np.newaxis]*self.soc * (g4b[:,np.newaxis]*2 * np.cos(kkp)/kkp**2 + g5b*((6*np.cos(kkp)-6)/kkp**4 - 3*np.cos(kkp)/kkp**2))
