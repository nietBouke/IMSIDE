...
def NewtonRaphson_check(self, init, version):
    ...
    #build the jacobian
    #subtidal part
    jac_tot = 0*self.jaco_subtidal(sss, self.st_all , self.ii_all , version) \
            + 0*self.jaco_bnd_subtidal(sss, self.st_all , self.ii_all , version)
    ...
    for i in range(len(self.tid_comp)):
        ...
        #add to jacobian
        jac_tot += 0*self.jaco_tidal(sss, tid_geg, self.ii_all , version)
        jac_tot += 1*self.jaco_bnd_tidal(sss, tid_geg, self.ii_all , version)


    def sol_func(ans):
        sol_tot = 0*self.solu_subtidal(ans, self.st_all , self.ii_all , version) \
                + 0*self.solu_bnd_subtidal(ans, self.st_all , self.ii_all , version) 
        ...
        for i in range(len(self.tid_comp)):
            ...
            sol_tot += 0*self.solu_tidal(ans, tid_inp, tid_geg, self.ii_all , version)
